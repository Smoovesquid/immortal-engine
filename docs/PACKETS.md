...
### INK-WRECK-1 — wrecked furniture RENDERS wrecked (2D ink live-state overlay)  ·  Phase 4 (the face) / map-fidelity  ·  **QUEUED 2026-07-09 (FUNC-MINIS-1 follow-up; Tim: queue, do not start).** The plan ink still draws a destroyed piece intact (b144: barrel gone from world truth + cover + blocking, still inked on the local map). Live-state source is ready-made (authoredFurniture.isFurnitureDestroyed / destroyedAuthoredPieceIds); the renderer’s furniture draw (LocalMap / floorPlan consumers) needs the overlay: ABSENT ⇒ debris mark or nothing; WRECKED ⇒ greyed/broken glyph. Map-fidelity law applies (a map fix isn’t DONE until it registers on the player map, verified live). Renderer lane — parallel-safe; pairs naturally with FURN-PARITY-1 but must not wait on it (authored pieces alone are worth the ink truth).
