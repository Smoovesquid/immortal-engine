...
- **HOUSE-BUILDER — ✅ LIVE + iterating** (`public/house-builder.html`, standalone dev tool, no engine change, no game-version bump; served at `/house-builder.html`). Drag-drop floorplan editor: rooms (box / round / per-wall bowed), **freeform wall segments** (any shape), **wood vs stone** walls (engine `structureMaterial.js` families, poché-differentiated), **doors + windows** (engine glyph language, snap to straight/freeform/**curved** walls, **resizable** by dragging along the wall), **tunnels** linking rooms (exported as `handDrawnInterior` corridors), furniture (bed/barrel/cookpot/dresser), **zoom+pan over a 120×90 world** (castle / mega-dungeon scale, Fit view). Export/Import `house-builder/v4` — engine-aligned shapes throughout (rooms shape+material+curve, walls, openings x/y/angle/len, corridors pts/w, furniture ux/uy). **NEXT engine-side:** a LOADER that mounts an authored structure at a node + walkable/narratable in play (rooms/round/material/furniture line up today; freeform walls, per-wall curves, sized/angled openings, tunnels are ahead of the engine model — future loader work). `cookpot` needs an engine furniture-catalog entry (bed/barrel/dresser exist). Memory: [[project_house_builder_authoring]].
...
- **NPC-BUILDER — SHELVED by Tim 2026-07-05 ("we can shelve the idea until we're done with the building builder").** A second builder for NPCs: choose stats/attributes (chargen-shaped) + a text area to drop the NPC's RAG-file data (→ a `server/rag/corpus/*.json` persona, the FACT-1/demoFigures authored-NPC path — see [[project_house_builder_authoring]] and the Carl fold-in flow). Do NOT start until the building-builder work is called done. Park; revisit on Tim's word.

### CONSEQ-1 — threads actually age (normalizeThread stops eating age/objective/trajectory)  ·  Phase 0/1 seam  ·  **QUEUED 2026-07-05 (found by the SL-5 lane; fix drafted + simulation-verified, NOT yet applied — engine lane)**
