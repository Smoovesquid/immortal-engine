...
- **DX-2 · wire the XCOM combat core into the live path.** **`engine/tactical/combat.js` ALREADY IS the XCom cover/hit core** — `coverAt()` (directional cover/flank), `hitChance` (cover+range+flank), deterministic `resolveAttack`. The **live D&D path** is `engine/combat/combatResolve.js` → `resolveCombatTurn` using `computeAttack/computeAC` (`engine/gear/gearProps.js`) + traitHooks/bossActions. **Prerequisite found (2026-06-28 audit): the live combat path is NOT positional yet** — `combatResolve` has no cell/x,y; `tactical/combat.js` is a separate dark module that takes a grid. So step 1 is **add committed positions to the combat state** (schema/version-bump — the propose/commit spatial resolver, MAP_PATH Phase 2.1–2.2), THEN feed cover→AC / flank→advantage into `computeAC`/`computeAttack`, then surface via DX-1's read. **This is a multi-step, determinism-sensitive engine project — NOT a quick edit.** Intended for the Codex lane, but **Codex CLI is not installed in this environment** (`/codex` skill unavailable), so it awaits either a Codex worker or a focused homebase session run against the combat gates. Do not rush it.
