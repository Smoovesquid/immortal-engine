...
keeps me honest about it. Companion to `docs/PLAYTEST_PROTOCOL.md` (the standing rule)
and `docs/PROSE_TO_WORLD.md` (the north-star depth spec).

---

## The bar (definition of done for the whole mechanic)

> Any reasonable thing a player would say to a DM produces — **AI-off** — a grounded,
> coherent, **visible** response that reflects the real world state and the true
> outcome, with no dead-ends and no abstract filler. **AI-on** makes that same
...
### Stage 1 — C: Movement & transitions  *(starts with the "go outside" bug)*
- **Scope:** "go outside" / "leave" / "enter the X" / "go to the well" / cardinals all
  resolve to real location changes with correct interior↔exterior state and grounded prose.
- **Lives:** `engine/playloop.js` movement gates, the place/interior system, `map/`.
- **Done when:** every movement phrasing either moves the player (with prose naming the
  destination) or clearly explains why not — never silently no-ops, never abstract floor.
- **Severe playtest:** in/out of buildings repeatedly; "go outside" from inside and from
  outside; "go to <named place>" for real and fake places; cardinals at edges; move →
  examine new place → move back (does it remember?); garbage destinations; rapid sequences.
