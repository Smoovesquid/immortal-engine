...
2026-07-16 — Worker (Opus, DEATH-TRUTH-1d worktree) — **DEATH-TRUTH-1d LANDED: people become places — the drawn spot IS the canonical spot (v0.48.0 b169)**
- **What (Tim's ruling, "proceed"):** MAP-REAL (*the drawn world IS the simulated world*) now extends to PEOPLE. The position the map draws a person at becomes their canonical engine position; the `NODE_JITTER` placeholder dies. This is the MAP-EGRESS-1 move (building scatter → engine truth) one level up — the third application (DEC-1 footprints, MAP-EGRESS-1 buildings, FURN-PARITY-1 furniture). It unblocks DEATH-TRUTH-1c's *picture* half: a canon-true corpse now draws where it died, matching the fight the player watched. Folds in 1e (corpses outside settlements).
- **The bug (1c's evidence, reproduced):** every outdoor entity was placed by `placeNearNode` — node centre ± 50 cells (a ±250 m box), blind to the drawn village. At Trader's Camp the buildings span place-unit uy −2.4…21.7, yet every NPC's canon projected to uy 52…70 — 0/… inside its own settlement. The renderer drew people at uy 9…15 (its own road-scatter), so canon and picture were **decorrelated**; routing a corpse through canon drew it ~230 m outside the village.
- **The fix — engine (Phase 2):** the renderer's outdoor people-scatter is PROMOTED into `engine/world/settlementScatter.js` (byte-identical to the old draw — same `shown` slice, same continued settlement rng, same `pushClearOfBuildings`; proven over 20 seeds × 2 frames). `placementForWorld` canonises each outdoor settlement occupant at the exact place-unit the sheet scatters them to → `placeUnitToRegionCell` → integer region cell. The player's outdoor settlement canon = the drawn lane entry (the keystone: the combat board's world origin pins to the player's canon, so grounding the player makes every death fact sheet-true). Wilderness nodes (no layout): player = node-centre cell, others = a small bounded WALKABLE near-node scatter. `placeNearNode`/`NODE_JITTER` deleted.
- **The fix — renderer (Phase 3):** the picture READS canon. New `regionCellToPlaceUnit` (the inverse of `placeUnitToRegionCell`, beside it, U719 round-trip). `playerTokenPlaceUnit` region branch flipped from fallback → projection (the stale "NO shared coordinate lattice" comment corrected — MAP-EGRESS-1 built it). Living NPCs draw from their canonical cell; corpse tokens draw from `remainsAtNode`'s `loc.pos` via the same inverse — so a living villager and the body they become share ONE coordinate exactly. Interior corpses draw at the struct-frame cell the fact froze (cell / PLACE_WU), room-scatter only as the degrade.
- **The fix — wilderness (Phase 4, the 1e fold):** a body on a NON-settlement node draws too — frame-free through `regionCellToWu` (node-anchored) on the 3D diorama (`placedTokenModel` corpse path + `render3d` gate opened for corpses) AND the 2D overworld sheet (`oneMap` node loop). Buildings/terrain ink stay gated by their own conditions.
- **Determinism / re-pin:** outdoor NPC canon values change (settlement-true, not jitter), so the boot worldHash moves ONE documented step — tallow `2fdf7b82…→1483b7da…` (U454-E), loaderDemo `8e5992e4…→a2bb740b…` (U713-E). Pos SHAPE unchanged ⇒ **no WORLD_VERSION bump, no save migration** (old saves keep their stored pos via the keep-if-consistent rule; new worlds are true). Replay determinism (U19/U21/U22/U27/U30) green; two fresh OS processes agree. The ≤ half-cell (≈2.5 m) snap from the integer-cell lattice is the accepted cost (§3f) — U494 proves no token lands in a wall.
- **Tests:** U714 (faithful promotion, byte-for-byte vs the old formula) · U715 (canon lands INSIDE the village, with a jitter falsifier) · U716 (a killed NPC's corpse on the EXACT square it stood; monster corpse at its loc.pos) · U717 (player grounds at the lane entry, board origin pinned) · U718 (wilderness corpse draws, gate killed) · U719 (lattice round-trips within a half-cell; no token in a footprint). Full suite green (11473). U576a/U576-oneMap carve-outs extended (the established per-packet pattern). 5 screen goldens re-blessed (position shift 0.78–5.27%, no degradation). Live receipt: seeded outdoor-corpse save on the worktree build — in-browser the Dire Wolf corpse draws at (12.15, 21.01) from its canon cell (−801,−392), npc_0's corpse at (9.65, 6.01) where it stood, player at canon (5.9, 23.51); the settlement renders coherently (no clumping/walls).
- **Files:** `engine/world/settlementScatter.js` (NEW), `engine/world/settlementLayout.js` (+regionCellToPlaceUnit), `engine/map/spatial/tacticalPos.js` (grounded regionPos, jitter deleted), `public/map/placeFromNode.js` (canon-read player/NPC/corpse), `public/map/LocalMap.js` (interior corpse cell), `public/map/drawModel.js` + `render3d.js` + `oneMap.js` (wilderness corpse), `tests/U714…` (NEW), U454/U713/U576 re-pins, 5 goldens, `package.json`/`public/v1.js` (v0.48.0 b169).
- **Residual (honest):** interior LIVING people still draw at a room-scatter angle (only the interior CORPSE reads the true cell) — pre-existing, not a regression; the "corpse where it died" bar is met. Outdoor overflow past the drawn cap and indoor-unplaceable NPCs use a bounded walkable near-node scatter (invisible; never the old ±250 m jitter). The crisp fallen-mark pixel at max zoom wasn't isolatable live (player-centered camera, no pan) but the in-browser token data is definitive.

2026-07-05 — Worker (Opus, structures serial worktree) — **LOAD-2 LANDED: a WHOLE hand-drawn building walkable at a real node — all rooms, doors→doorways (engine-leads)**
- **What (Tim's decision — ENGINE LEADS):** LOAD-1 loaded ONE room; LOAD-2 makes a WHOLE hand-drawn building walkable — ALL its rooms, connected by its authored DOORS as reciprocal-compass doorways — so you walk in from the map and move room-to-room. "The castle you drew is the castle you explore," minimal version. Brief: `docs/briefs/LOAD-2-multiroom-realnode.md`. EXTENDS the LOAD-1 loader (`engine/structures/authoredStructure.js`), does not fork it.
- **The crux — doors → adjacency (how):** the house-builder tool records a door as `{kind:'door', x, y, room}` — a SINGLE room tag (its nearest-wall sample), so the OTHER room a doorway joins is never written. `roomsAtOpening` recovers it by geometric boundary membership: a door on a SHARED interior wall sits on TWO rooms' rects → that pair is a topology **edge** (the doorway the player goes through); a door on an OUTER wall sits on ONE room → the exterior front door. The engine's `interiorCompassLayout` (topology.js) then assigns each edge a reciprocal compass slot (A north→B ⇒ B south→A) — the loader SUPPLIES the edges, the engine assigns the directions. Door directions in the plan are read back FROM that same compass layout, so the drawn door and the direction movement resolves always name the same exit (no desync between `getInteriorView`'s "Exits: north" and the door record).
- **Never orphan / never soft-lock (two fallbacks):** (1) where the drawn doors leave a room's connection ambiguous, **abutment fallback** — two rooms sharing a wall segment get an edge (you drew them touching, so they connect even without a drawn interior door); (2) any room still unreachable from the entry is **repaired** to its nearest already-reached neighbour, so the graph is ALWAYS connected. Both are recorded on a non-enumerable `__loaderInfo` (roomCount, edgeCount, `abutted`, `repaired`) — flagged, never hidden — without entering the hashed shape.
- **Maps DOWN (engine-leads):** ALL rooms → topology rooms `room:<structId>:<n>` (entry canonicalized to `:1` so interiors.js drops the player there). Each room carries its authored **role** as a `role:<role>` tag when it names a KNOWN engine role (hall→hearthroom, a side room→bedchamber, another→scullery) so `roomDetail` reads it and "look around" is truthful per room; an unknown/absent role falls to a cottage default by position (entry→hearth room, others→bedchamber). Material 'stone' anywhere → stone shell else timber. `authoredPlan` keeps Tim's DRAWN geometry (room positions/sizes) verbatim (floorPlan's MR-2c override returns it), with role-appropriate furniture per room so the map + prose agree.
- **DEGRADED (noted, not represented — future):** freeform/bowed `walls[]`, per-wall room `curve`s (round rooms use the engine round shape), `tunnels`/`corridors` (a tunnel-only link degrades to the abutment/repair pass, not a modelled passage), `secrets`, sized/angled door openings (`len`/`angle`), the DRAWN per-piece furniture POSITIONS (furniture is regenerated from the room role — a corner bed reads as "a bed" at the role's canonical layout), multi-FLOOR (single-storey; a multi-floor export degrades to one storey — noted).
- **The attach seam (brief §3 — real node, behind a seed):** `applyGeneratedStructuresForNode.js` gains `maybeInjectLoaderDemo2` (seed-gated on `loaderDemo2`, mirroring LOAD-1) + a **node-keyed registry** `AUTHORED_AT_NODE` mapping a node PREDICATE → an authored house (`authoredHouseForNode(node)`). The registry keys by node NAME (node ids are seed-derived, not stable strings), so the three-room warden's cottage lands on the REAL **Crowfoot Camp** slice node regardless of the slice's hashed id. **Every non-`loaderDemo2` seed returns before any of this runs — the default game (incl. the `aldermere` slice that HAS Crowfoot Camp) is byte-identical: 0 authored buildings, worldHash unchanged.**
- **Determinism proof:** the loader is PURE (no `Math.random`/rng/I/O) — same export → byte-identical structure (U521); authoring order doesn't matter (normalizeTopology sorts); a world carrying the building hashes stably across an export/import round-trip (U521, the U21 invariant); the SAME walk from the SAME seed reaches the SAME worldHash. **No `WORLD_VERSION`/`STRUCTURE_SCHEMA_VERSION` bump** (reuses existing shapes — `topology.edges`, `authoredPlan`, `doors`).
- **Tests (U518–U522, LLM-off, 34 subtests pass):** U518 a three-room export → all rooms in topology with the right roles + role-driven furniture + ensureStructures round-trip. U519 door→adjacency: interior doors become edges, the compass is reciprocal, a no-doorway direction is a wall, plan door dirs match the compass, no orphan; abutment fallback + orphan repair proven on crafted exports. U520 walk it: enter→Hall→each side room through its doorway→back, truthful narration, real graph (describeInteriorLayout roomCount=3), egress from anywhere (no soft-lock). U521 determinism. U522 malformed fails loudly (6 cases) + fully-disconnected repairs to connected + default boot byte-identical + the node registry maps Crowfoot Camp.
- **Live-verify (LLM-off, real `playerMove`):** boot `loaderDemo2` → wake inside the cottage → "go outside" (real map node, real exits) → "go inside" (the Hall) → "go to the bedchamber"/"go to the scullery" ("You step through into the…", reciprocal doorway back, no invented geography) → return to the Hall. PLUS a REAL-SLICE-NODE section: the building attached at the real Crowfoot Camp node on the real `aldermere` slice map, entered from the map and walked room-to-room. Transcript: `docs/playtests/loader/LOAD-2-liveverify-llmoff.txt`. Honest note: base LLM-off interior "look around" prose is terse ("The only way on lies east") — the rich weave is the LLM layer's job; the engine STATE feeding the DM is fully truthful and grounded.
- **Residual risk:** (1) door direction is the engine's planar compass, not the drawn wall's real-world bearing — a door on a room's east wall can navigate as "south" if the compass BFS placed it there; consistent everywhere and reciprocal, but "go east" won't always match the drawn side (acceptable — the engine owns navigation; noted). (2) A tunnel-linked (non-abutting) pair of rooms connects only via the repair pass (nearest-neighbour), not as a real modelled tunnel — fine for connectivity, not for tunnel semantics. (3) The node-keyed registry attaches at Crowfoot Camp only under the `loaderDemo2` seed (never the default slice) — wiring it onto the shipping slice is a content decision (flip the gate), deliberately not taken here.
- **Green:** my 5 files **34/34**; default boot byte-identical (aldermere/tallow → 0 authored, worldHash stable). No `WORLD_VERSION` bump. `package.json`/`public/v1.js` version left for integration (Basecamp owns `public/` + the front-door build line).

2026-07-05 — Worker (Opus, structures serial worktree) — **LOAD-1 LANDED: the smallest loader — one hand-authored room becomes walkable (engine-leads)**
- **What (Tim's decision, 2026-07-05 — ENGINE LEADS):** the smallest thing that proves the PATH from a hand-drawn floorplan to a room the player can walk into. A minimal `house-builder/v6` export (ONE room + material + door) is mapped DOWN onto the engine's existing interior model; everything the engine can't yet represent is IGNORED/DEGRADED (noted below). Goal met: from the demo seed the player is INSIDE the authored room at boot, can "look around" (it narrates as a truthful cottage quarters — one room, timber, real furniture, NO invented geography), "go outside", and "go inside" again. Brief: `docs/briefs/LOAD-1-smallest-loader.md`.
- **The seam I plugged into:** MR-2c had already built the authored-plan pipeline (`authoredPlans.js` `makeAuthoredStructure`, the authored-override in `applyGeneratedStructuresForNode.js`, `roomState.js`/`roomDetail.js`/`structureMaterial.js` reads, `doors.js` `deriveDoors` — the whole enter/look/exit path in `interiors.js` + `playloop.js`). What was missing and this packet adds: (1) a **`loadAuthoredStructure(json,{nodeId})`** public entry that consumes a house-builder export DIRECTLY (not just a pre-registered id), and (2) a **live-reachable demo** so a player actually reaches the room.
- **New `engine/structures/authoredStructure.js`:** `loadAuthoredStructure(json,{nodeId})` — validates a `house-builder/v6` (or v5) export and builds a valid `ensureStructures`-shaped structure `{kind,nodeId,anchors,topology,surfaces,tags,buildingType:'cottage',authoredPlan}` from the FIRST room. Pure + deterministic (no rng, no I/O). Malformed JSON THROWS LOUDLY (`authoredStructure: malformed authored plan: …`) — never silently corrupts state. Accepts a parsed object OR a raw JSON string.
- **Maps DOWN:** ONE room (the first), id canonicalized to `room:<id>:1` + tagged `entry` (interiors.js drops the player here); **role** = `room.role` else `quarters`, carried as a `role:<role>` tag; **material** `timber`/`stone` → the shell (else timber); the room is enterable because the engine tail (`deriveDoors`) authors exactly one exterior front door on the entry room. `buildingType:'cottage'` means furniture + the material line flow through the existing cottage read stack with no new machinery.
- **Degraded — future (noted, not represented):** extra rooms (only the first loads — multi-room already has MR-2c's path), freeform/bowed `walls[]`, `tunnels`/`corridors`, `secrets`, `windows`, room `curve`s, sized/angled door openings, multi-floor. Each is dropped, not errored.
- **One additive edit to `roomDetail.js`:** an authored `role:<name>` room tag now wins over the building-blueprint default **only when it names a KNOWN role**. So a single hand-drawn room marked `quarters` reads as a Guest Room (with a bed), not the cottage entry's hearth room. ADDITIVE — every procgen room carries no such tag, so its role/furniture selection is byte-identical (the material/furniture/worldHash tests confirm no drift).
- **The demo wire-in (seed-gated):** `applyGeneratedStructuresForNode.js` gains `maybeInjectLoaderDemo` — when `world.meta.seed === 'loaderDemo'` it loads `packs/base/structures/authored/loader_demo.house.js` (a fs-free `export default` twin of the test fixture) and attaches it at the current node, id `authored:<node>` (sorts before `stgen:…`, so `beginAdventure`'s enter-`#1` opens the game inside the authored hut). **Every other seed returns before any of this runs — the default game is byte-identical.** To trigger: boot a world with seed `loaderDemo`.
- **Determinism proof:** authored content is FIXED data (like a pack) — no `Math.random`, no `WORLD_VERSION` bump (reuses existing shapes). Two independent `loaderDemo` boots hash identically (U516); an enter→exit→enter→exit walk stays hash-replayable and invariant-clean (U514/U516); two ordinary boots are byte-identical off-seed (U517).
- **Tests (U513–U517, LLM-off):** U513 the fixture → a valid structure (topology normalized, one entry room, authoredPlan, ensureStructures round-trip; fixture == demo module). U514 enterable/exitable via the real `interiors.js` seam, one room, NO phantom geography, no soft-lock. U515 furniture (a bed) + the timber material line narrate (stone-wall phrases forbidden); a stone fixture selects the stone shell. U516 loader purity + demo worldHash stability. U517 malformed input fails loudly (11 cases) + the default boot is byte-identical (no authored/loader structure off-seed). **30 new tests pass.**
- **Live-verify (LLM-off, own driver, real `playerMove` — the server's turn path):** boot `loaderDemo` → open inside "Guest Room" (quarters/cottage/timber, 1 room, furniture listed) → "go outside" (region frame; both structures visible) → "go inside" (back in the authored room) → invariants clean at every step. Transcript: `docs/playtests/loader/LOAD-1-liveverify-llmoff.txt`. Honest note: base LLM-off "look around" prose is terse (the rich furniture/material weave is the LLM layer's job, out of scope); the engine STATE feeding the DM is fully truthful and grounded, with no invented rooms/stairs.
- **Green:** clean isolated full suite **10095/10095**, my 5 files **30/30**, determinism U19/21/22/27/30 green. (`npm run check` showed a single 1-of-10208 flake under its heavier parallel load — the documented U381-class machine flake; the isolated `node --test tests/*.test.js` run is 0 fail, confirmed twice, and none of the failing test is mine.) No `WORLD_VERSION`/`STRUCTURE_SCHEMA_VERSION` bump. `package.json`/`public/v1.js` version left for integration (Basecamp owns `public/` + the front-door build line).
