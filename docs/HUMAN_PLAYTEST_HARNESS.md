...
**Critical correction (2026-06-24): test the REAL DM, not the skeleton.** Phases 1–3 drove `playerMove` only — i.e. the deterministic `gen:s/m/f` atmosphere filler (`playloop.js:5917`) the engine itself flags *"NEVER ship"* — and **never `/api/narrate`**. So the oracles judged a game nobody plays, and they only ever check *consistency* (does prose contradict committed state), never *quality* (is this a real DM). A saturation run that found nothing then printed "clean," laundering *no signal* into false confidence. **Fixed:** `scripts/playtest-harness.mjs --real-dm` routes every turn through the live DM in-process (`augmentNarration`, no server needed) **and** runs the **Tier-2 quality judge** (`engine/harness/qualityJudge.js`) — a rubric-atomic, evidence-anchored THE_TABLE_TEST critic (resolves-the-action · specific-not-filler · a-real-DM-would-say-it). Proven live on `tallow`: it caught a stat-dump (*"forty-four souls make their living"*) and a non-resolution the deterministic oracles sailed past. Quality findings are tagged `quality-*` — **discovery / human-triage, NEVER auto-fixed** (only deterministic findings gate the fix loop; the guard is `isAutoFixable`). The empty-run wording now distinguishes a real coverage-bounded clean (`--real-dm` active) from an INCONCLUSIVE skeleton run. Due-diligence (2026 SOTA): rubric-atomic + evidence-anchored + splitting verifiable-consistency from open-ended-quality is the consensus (Autorubric 2603.00077, RULERS 2601.08654; "Is Your LLM a Good Game Master?"); we hold an edge because we own the Canon Log as ground truth.
...
3. **The oracle bank** (per `HARNESS_USAGE_STRATEGY.md` §"oracle stack"; THE_TABLE_TEST is the spec):
   ...
   - **Tier 2 — LLM quality judge (BUILT 2026-06-24, `engine/harness/qualityJudge.js`):** rubric-atomic, evidence-anchored THE_TABLE_TEST critic over the REAL DM transcript (one cheap call/session, no-CoT per Vol 14, decoupled model). Catches the open-ended "not a real DM" class the deterministic oracles can't — filler, stat-dumps, non-resolutions. Findings are `quality-*` severity (discovery / human-triage, **never auto-fixed**). Requires `--real-dm`. *(Consistency vs. Canon Log stays the deterministic floor; this is the quality discovery tier.)*
