...
- **Verification:** quiet-machine `npm run check` GREEN — suite **9588/9588**, convergence **124/124**, determinism U19/21/22/27/30 green. **U381 adjudicated:** fails only under concurrent parallel suites (port contention), passes quiet — hardening packet queued (PACKETS §TABLETOP/HARNESS). Live map-fidelity screenshots at land (interior occupants + outdoor token + build line).
