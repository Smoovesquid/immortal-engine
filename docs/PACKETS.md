...
### FUNC-MINIS-1 — Builder-placed furniture becomes engine truth (Tim's commission 2026-07-09)  ·  MAP-REAL / make-correct pillar  ·  **✅ LANDED v0.35.0 b144 (Basecamp/Fable main-checkout, Tim-authorized in-chat brief 2026-07-09).** Hard bar: "if a thing can be placed, the world must believe in it." Root was documented in the loader's own header: authored rooms REGENERATED furniture from role — three drawn barrels in a bedroom didn't exist. Now drawn pieces ride topology+authoredPlan verbatim (count/kind/identity/drawn position; no role substitutes, no injected crate) and seed real node.furniture pieces once-per-structure (no resurrection; ROM-4 unique names; explicit material/hardness). Live-state cover AND walk-blocking subtract destroyed pieces (wreck = terminal state | parts exhausted | twin absent) — the SAME stored record the smash mutates. Four kinds delivered: barrel (smash→salvage board E2E), bed (intact placed bed = settlement-band long rest; wrecked bed = nothing), cookpot+dresser (NEW FURN kinds — engine had neither; catalog 39→41). Placed pieces lead the room survey. Tests U659–U664 (33 asserts, red-first); suite 11001/0; check GREEN ×2; live v1 receipt (board+nails in the pack panel). Architecture note `docs/briefs/FUNC-MINIS-architecture.md` (contract + future-kind recipe). DEVIATIONS flagged: STRUCTURE_SCHEMA_VERSION NOT bumped (procgen byte-identical; bump would churn every stgen id) · one acceptance-required line in escapeCombat.js. **ACCEPTED by Tim 2026-07-09** (both deviations blessed: no schema bump — procgen byte-stability/save continuity beats a ceremonial bump; the one-line escapeCombat touch — acceptance #6 required it). DM-GATE-1b stays separate, do not fold in. Follow-ups queued (NOT started, Tim’s word) as **OBJ-PRESENCE-1 · COOK-1 · FURN-PARITY-1 · INK-WRECK-1** — rows below.

### OBJ-PRESENCE-1 — "is the barrel still here?" / "where's the barrel?" answer from room OBJECTS, never NPC presence  ·  Phase 0 (THE DM TEST)  ·  **✅ LANDED v0.35.3 b147 (Basecamp/Fable main-checkout, Tim-authorized in-chat brief 2026-07-10; scope corrected mid-brief to also cover "where's the X" — leaving it as a documented known-failure was "the same player-trust loop").** ROOT WAS A META-GATE INTERCEPTION, not playloop routing as first assumed: `META_NPC_PRESENCE` (gracefulAdjudication.js) matches "is (that|this|the) \w+ (gone|left|still here/around/there)" blind to person-vs-object, answering with NPCs before playloop's already-correct `objectPresenceTarget` ever runs. Reproduced general (authored AND procgen furniture). SECOND bug found mid-diagnosis, fixed in the same packet: even the WORKING phrasing said "Yes — there's a barrel here: battered apart" for a present-but-wrecked (rulings-lane) piece — shipping routing-only would have made that case worse. Fix = two shared helpers (`roomFurnitureNounMatch` gates a noun against roomDetail's FURN vocabulary; `roomObjectState` reads objectsHere → intact/wrecked/absent via isFurnitureDestroyed) wired into TWO call sites: the NPC-presence defer (mirrors the EXISTING carried-item H-65 precedent, same shape) and `answerObjectLocation` (previously carried-items-only, unconditionally called any miss "invented"). isMetaQuestion untouched; engine/playloop.js untouched. Tests U677–U682 (24 asserts); suite 11,070/0; check GREEN; live receipt (real smash → "No — no barrel here." / "No barrel here to speak of — not in your hands, not in the room.", board+nails visible in the pack panel, screenshot). **OBJ-PRESENCE-1b ✅ LANDED v0.35.4 b148 (same day, Tim-authorized micro-packet — "close it before WIN-LOOK-1"; Tim's own sizing: "tiny but engine-facing parser fix").** Closed the one honest gap above: `objectPresenceTarget`'s anchor required the article immediately after "there" — an inserted "still" broke that adjacency, falling the turn to the generic room-survey floor (confirmed via the same isMetaQuestion===false empirical proof, so no gracefulAdjudication.js-only fix could ever have reached it). Fix = ONE line in engine/playloop.js: `(?:still\s+)?` tolerance in the anchor. gracefulAdjudication.js/isMetaQuestion untouched, no broadening. Test-writing catch: an initial control wrongly assumed this function furniture-vocabulary-gates non-object nouns the way OBJ-PRESENCE-1's NEW gracefulAdjudication.js helper does — it never has; fixed the test to assert the real invariant (parity between "is there a ghost here?" and "is there still a ghost here?", both pre-existing "no ghost here" honest declines) instead of inventing a new rejection rule. U683 (6 asserts) + U680's prior "known gap" test flipped from red-by-design to confirming-the-fix (its own comment predicted this outcome). Full object-presence suite U677–U683 30/30; neighborhood 95/95; full ladder GREEN. No WORLD_VERSION. **The whole object-presence player-trust loop is now closed — all three phrasings ("is X still here", "where's X", "is there still an X") answer honestly, in every state (intact/wrecked/absent), for both authored and procgen furniture.**

### BUILDER-OBJ-1 — the Builder's supported object palette: place it ⇒ the game believes it  ·  MAP-REAL / make-correct pillar  ·  **✅ LANDED v0.36.0 b151 (Basecamp/Fable main-checkout, Tim's 2026-07-11 autonomous dispatch — scope+stop-rules in the dispatch itself; engine edit pre-authorized there, brief rendered in-chat, marker touched and flagged).** Hard law: "If it is placeable in the Builder, the game must believe in it. No decor-only minis." FUNC-MINIS-1's chain was already kind-agnostic — the gap was HONESTY at the edges: only 4 of 41 kinds had explicit Model A physics (a placed lantern minted material 'fire' hardness 0 via the Model B render-material fallback; a hearth was hardness-0 fire; wardrobe/nightstand/crate weren't containers you could open), the Builder offered 4 kinds first-class (everything else buried in search at a wrong 1×1 footprint), and nothing said which kinds have GLB art. Delivered: (1) `KIND_PHYSICS` +12 explicit rows (chest, crate, shelf, table, chair, nightstand, wardrobe, rug, runner, lantern, candles, hearth — data only, no logic change; FURN catalog untouched ⇒ zero procgen drift); (2) 16-kind first-class palette in house-builder.html with real footprints + per-kind status chips (**real object** · **real object + has art (N GLB) — needs wiring** · **needs object contract**, the last blocked at runtime too); (3) NEW `public/map/propArt.js` — curated kind→confirmed-single-GLB registry (`wired:false` until a placed-piece GLB render path exists; treeAssets' cottage set + the armory trio → `rack`); (4) miniLibrary `sheet:true` flags carry the 07-10 sheet audit as DATA, and the Builder's Mini-library panel refuses them honestly ("needs split → object contract"); corpse category labeled engine-placed. Tests U685 (palette⊆FURN law, registry integrity, sheet-never-claimed) + U686 (all-16-kind contract: loader carry verbatim, flat/light fields, explicit physics red-first, seeding count/identity/idempotence) — 10/10; FUNC-MINIS U659–U664 untouched green; full ladder GREEN. Live receipt: palette chips + placed table/rug/lantern/hearth + library refusals, screenshots. Deliberately NOT done (labels mark them): GLB→placed-piece render wiring (`wired:false` — **done by BUILDER-OBJ-2, below**), cooking (COOK-1), object HP/AC (DM-GATE-1b), systemic fire, splitting the parked sheets. Prior-window 07-10 intake (armory trio + hearth sheets + audit) landed as its own attributed commit first.

### BUILDER-OBJ-2 — placed objects render with their confirmed GLB art  ·  MAP-REAL / make-correct pillar  ·  **✅ LANDED v0.37.0 b152 (Basecamp/Fable main-checkout, Tim's 2026-07-11 autonomous dispatch — brief rendered in-chat, no engine edit required so no marker touch).** BUILDER-OBJ-1 landed the engine-truth side of the Builder's 16-kind palette but left every propArt.js kind `wired: false` — "no placed-piece GLB render path exists yet." Tim's ask: "a placed table should look like the table GLB, not a placeholder." Delivered: `figures3d.js`'s `buildPropMini` now prefers a GLB (via `treeAssets.js`'s already-preloaded furniture templates, `glbPropMini`, gated on propArt.js's own `wired` flag — the single source of truth the Builder's status chips also read) before falling back to its old procedural box (which survives only for barrel/bed/chest/dresser); `GLB_KIND_ALIAS` maps the one differing name (`cookpot` → the `cauldron` GLB — a GLB never names the engine object). `drawModel.js`'s `PROP_MINI_KINDS` grew 4→9 to match (hearth/table/chair/cookpot/rug can now become 3-D minis at all — they never could before, a stale TT-PROPS-era exclusion this packet's commission directly supersedes). `PROP_TRUE_SIZE` gained the five new kinds' true sizes, including a new `axis: 'footprint'` option (yaw-invariant, via the existing `measureAuthoredFootprint`) for table/rug so a flat/wide GLB's near-zero height-axis read can't blow its scale up. Wired: barrel, bed, chest, dresser, hearth, table, chair, cookpot, rug (9/10 registry entries) — `rack` stays unwired (its art is on the lazy `miniLibrary` loader, a second async path, parked not built). House-builder's chip text updated to "real object · GLB rendered" per the brief. Tests: U687 (new — hermetic gating/fallback + source-contract proof, since three.js can't run headless here); U685/U480/U576a/U576e updated CONSCIOUSLY (each documents why in its own comment) since this packet legitimately grows propArt.js's wired set, PROP_MINI_KINDS, and PROP_TRUE_SIZE. Suite 11,104/0; `npm run check` GREEN. Live receipt: the tallow boot cottage's Hearth Room renders a detailed brick hearth (with chimney), an iron-banded chest, a flat rug, and a barrel as real sculpted GLBs (not procedural boxes); `window.__rendScaleAudit` additionally confirms `table`'s mini resolved (no procedural fallback exists for it, so its presence there is only possible via the GLB path). Deliberately NOT done: `rack`'s miniLibrary async loader, cooking (COOK-1), object HP/AC (DM-GATE-1b), splitting the parked multi-object sheets. Architecture note updated: `docs/briefs/FUNC-MINIS-architecture.md`'s BUILDER-OBJ-2 addendum.

### BUILDER-PREVIEW-1 — one-click "Preview 3D": the Builder draft opens in the game's own renderer  ·  builder-tools / MAP-REAL pillar  ·  **✅ LANDED v0.38.0 b153 (Basecamp/Fable main-checkout, Tim's 2026-07-11 autonomous dispatch; engine seam pre-authorized in the dispatch, brief rendered in-chat, marker touched and flagged).** Tim's ask: place rooms/furniture, click Preview, see the playable 3D/game-map rendering with GLBs — no export/import juggling, real save untouched. Delivered: (1) Builder header button **Preview 3D ↗** — runs the CURRENT draft through the SAME validate/finalize gate as Validate & Finalize (one shared helper; invalid drafts get the identical room-by-room explanation), stashes the finalized artifact at scratch key `ie.builderPreview` (never a save slot), opens `public/builder-preview.html` in a named tab; plus the plain-English goal line pinned atop the Builder sidebar. (2) NEW preview page = a THIN MOUNT of the game itself: registers the draft on the engine seam, boots `beginAdventure(newWorld({seed:'builderPreview'}))` (ephemeral — the page never writes ANY storage), mounts `renderContinuousMap` full-page with v1.html's three.js import map, pre-warms the GLB template pools before the first scene build (a static preview never takes a turn, so late templates would otherwise leave placeholder tokens forever), and auto-zooms into the diorama band. (3) The one engine edit (`applyGeneratedStructuresForNode.js`, additive): seed-AND-registration-gated `maybeInjectBuilderPreview` — **MR-2c SUBSTITUTION, not attach**: the draft adopts the boot node's first procgen candidate id so the settlement layout anchors it and the map draws it like any other building (the first cut attached a free-floating `authored:<nid>` structure — fully playable but INVISIBLE to the sheet: no place anchor, no world rect; found live, reworked same session). Attach remains only the no-candidate fallback. DIAGNOSED EN ROUTE: bare-specifier `'three'` needs the import map or `mountSlice3D` throws and continuousMap's `_failed` latch silently pins the map 2D (now a U689 assertion). Tests U688 (boot-inside-house via authoredPlan, furniture real at node, unregistered→procgen, deterministic replay, DEFAULT-SEED WORLDHASH ISOLATION with a house registered) + U689 (preview ships the same doc as finalize; Builder writes ONLY the scratch key; preview page is storage-read-only, no save.js import; banner honesty; import map present). Suite 11,113/0; check GREEN; live receipt: one click → PREVIEW banner → drawn Room 1 on the game sheet → tilted 3D diorama with the sculpted hero + barrel GLB + table GLB in the drawn room (hearth GLB confirmed mounted via `__rendScaleAudit`); localStorage after the whole flow = exactly `ie.builderPreview`. Deliberately NOT done: walking/turns inside the preview (it is a viewer, not a play surface), any renderer change, sheet splitting.

### BUILDER-PREVIEW-1b — the preview shows JUST your building, not the town it borrows  ·  builder-tools follow-up  ·  **✅ LANDED v0.38.1 b154 (Basecamp/Fable main-checkout, Tim's live report on b153: "the preview gave me a saltmarket town with NPCs and trees").** ROOT: BUILDER-PREVIEW-1's MR-2c substitution parks the draft in a real settlement building slot so it draws — but a settlement renders its WHOLE self (decorative town buildings + an outdoor NPC roster + two hardcoded groves ≈15 trees + a well, settlementLayout.js). FIX is renderer-only + opt-in, ZERO engine edits: a `bare preview` gate in `placeFromWorldNode` (public/map/placeFromNode.js — the single chokepoint both 2D ink and the 3D TT-PROPS diorama read for a node's buildings/tokens/terrain), keyed on `world.__builderPreview`. When set it (a) keeps only buildings whose structureKey resolves to an `authoredPlan` structure — the drawn house alone, no decorative town; (b) emits no outdoor-NPC tokens — no strangers; (c) returns `groves:[]`/`props:[]` — no trees, no well (road ribbon kept for ground orientation). The preview page (builder-preview.html) sets the flag on its throwaway world, islands the map to just the boot node (`nodes:[boot]`, no neighbours drift in on zoom-out), and renames that node to the house name so the 3D HUD titles it after the building, not "Saltmarket Town" (nodeType STAYS settlement — flipping it would trip render3d's wilderness tree/tent scatter). Default game byte-identical (flag never set outside preview; goldens + determinism U19/21/22/27/30 green). Test U690 (3): un-gated control still draws many buildings + groves (the bug), gated draws the authored building alone with no npc tokens / no groves / no props, and a default-seed world is never flagged. Suite 11,116/0; check GREEN; live receipt: draw a room + hearth/table/rug/barrel/chair → Preview 3D → the building on clean ground, hearth/table/barrel as GLBs, HUD reads the house name, no town/NPCs/trees; localStorage still exactly `ie.builderPreview`.

### BUILDER-PREVIEW-2 — Preview 3D renders the AUTHORED ROOM, not a settlement tile  ·  builder-tools / MAP-REAL pillar  ·  **✅ LANDED v0.39.0 b155 (Basecamp/Fable main-checkout, Tim REJECTED b154 with a new bar: "a giant empty settlement tile with the player and loose objects, not the actual house/building interior").** DIAGNOSIS: b153/b154 booted an engine world and rendered via the overworld/settlement diorama (renderContinuousMap). But that renderer draws architecture as INK on a flat ground sheet (TT-INK — `buildSettlement` deliberately stopped mounting 3-D wall geometry), so walls NEVER rise; the bare-preview gate removed the town but left a flat plane + floating furniture. The overworld renderer fundamentally can't show a room shell. PIVOT (Tim-authorized "escalate if you need to redesign preview rendering"): a DEDICATED interior renderer, `public/map/builderPreview3d.js`, fed the raw builder doc directly (no engine boot) — `buildPreviewLayout(doc)` is a PURE doc→world-space spec (rooms→floor slabs at metres; each room boundary→wall beams WITH a gap cut at every door opening; furniture→world position from the drawn ux/uy inside its room, carrying rotation; a person in the entrance room for scale; bounds for camera framing), and `mountBuilderPreview3D` is the thin Three.js consumer (floors, box-beam walls open-topped so an angled camera sees in, the GAME's own GLB furniture minis via figures3d `buildPropMini` true-sized in metres — 9k–16k-vert sculpts, a labelled box fallback for kinds without a mini, the game's `buildArchetypeFigure` person, a framed orbit camera, warm interior lighting). REVERTED the now-dead settlement-boot plumbing: the engine injection (`registerBuilderPreviewHouse`/`maybeInjectBuilderPreview` in applyGeneratedStructuresForNode.js — engine now byte-identical to pre-b153) and the placeFromNode.js bare-gate. Preview page rewritten: read scratch key → `mountBuilderPreview3D(stage, doc)`, storage READ-ONLY, no engine imports. Tests: U691 (5, NEW — pure layout: rooms→floors, door-gap walls, furniture inside its room at drawn pos, person in entrance, empty-draft→empty) + U689 (rewritten — same-doc-as-finalize, scratch-key-only writes, page uses builderPreview3d & no longer boots a world); DELETED U688/U690 (they tested the reverted engine-boot/bare-gate paths). Suite 11,113/0; convergence 135/135; goldens locked (engine revert = default game byte-identical); check GREEN. Live receipt: draw a 2-room cottage (hall+snug, shared-wall door, front door) + hearth/table/chair/barrel/bed → Preview 3D → recognizable 3-D building: two walled rooms, the front doorstep + interior doorway gaps, GLB furniture standing inside at the drawn spots (hearth glowing), a person for scale, framed on the house; no town/NPCs/trees; localStorage after the flow = exactly `ie.builderPreview`. Meets Tim's full acceptance bar.

### BUILDER-PREVIEW-3 — Preview 3D = your building alone on the GAME's own tabletop  ·  builder-tools / MAP-REAL pillar  ·  **✅ LANDED v0.39.1 b156 (Basecamp, 2026-07-13; Tim-authorized correction of b155, committed as its own scoped commit ahead of the object-physics arc).** Live receipt: imported an asymmetric 2-room cottage (Hall+Snug, front door + shared-wall door, hearth/table/rotated-chair/barrel/rotated-bed/chest) → Preview 3D → the building ALONE on the graph-paper tabletop, authored ink floorplan, the game's tilted north-up diorama camera, all 6/6 furniture as real GLBs standing on their ink marks with authored rotation (chair 90° · bed 90°); no town/NPC/player/terrain/trees, no raised walls (walls are ink), no colored slabs, no block placeholders. Banner reflects runtime facts ("6/6 placed GLBs") and correctly expanded to a truthful "GLB failed — no placeholder shown" warning during the one build where a loader race left 5/6 unresolved (fixed: the adapter now polls treeGLBsReady() so the whole GLB pool is warm before the static mount). Runtime audit: every wired kind (barrel/bed/chest/hearth/table/chair/dresser/cookpot/rug) resolves a GLB; rack (unwired) honestly does not. Save-safety: only `ie.builderPreview` added; seeded fake real-save keys byte-identical. Console clean, all GLBs 200. `npm run check` GREEN (convergence 135/135 · suite 11,129/0 · screen-truth goldens locked = default game rendering byte-unchanged). Tim's bar after b155: the dedicated dollhouse renderer drew its OWN look — raised box-beam walls, colored floor slabs, a scale person, a bespoke SE-corner camera, and BoxGeometry fallbacks when a GLB was missing. That is a THIRD visual direction, not the game. New target: click Preview 3D → **your building ALONE**, on the game's **graph-paper/tabletop surface**, showing the **authored ink floorplan**, at the game's **normal tilted 3D map POV**, with **real GLB furniture at the authored positions + rotations**, auto-framed — and NOTHING else (no settlement/town, no terrain/roads/groves/scenery, no NPC/player/scale figure, no raised dollhouse walls, no colored slabs, no fake geometry, no generic-block placeholders, no save mutation).
- **objective:** re-point Preview 3D onto the REAL gameplay 3D renderer via a thin projection adapter — NOT `renderContinuousMap` (proven in b153/b154 to pull in settlement/world/player), and NOT the b155 dedicated dollhouse renderer (kept behind no toggle). Shared boundary = `mountSlice3D()` in `public/map/render3d.js`.
- **flow:** finalized Builder doc → thin projection adapter (`builderPreview3d.js`) → canonical graph-paper/ink canvas (via `handDrawnInterior.js` + the shared plan model) + authored GLB positions/rotations + isolated building bounds → `mountSlice3D` preview projection → gameplay camera, lighting, tabletop, prop presentation.
- **allowed_files:** `docs/PACKETS.md`, `public/house-builder.html`, `public/builder-preview.html`, `public/map/builderPreview3d.js`, `public/map/render3d.js`, `tests/U689.builderPreviewContract.test.js`, `tests/U691.builderPreviewLayout.test.js`, `tests/U692.builderPreviewTabletop.test.js`.
- **forbidden:** runtime authored-furniture rotation persistence outside the preview (use the finalized RAW furniture data for `rot`); new furniture art; world generation; save-format change; settlement-preview behavior; a general renderer rewrite; WORLD_VERSION bump; unrelated cleanup. No `save.js` import / `newWorld()` / `beginAdventure()` / world registration / real-slot write from the preview; scratch key `ie.builderPreview` only.
- **invariants:** normal gameplay rendering byte-unchanged when `opts.preview` is absent; determinism untouched (renderer is a pure view); no `Math.random` in render3d.js; GLB truth = a `wired` kind requires `userData.glb === true`, never a fabricated block.
- **test_plan:** rewrite U689 (scratch-only handoff · no save imports · no world boot · thin gameplay-renderer wrapper · persistent plain-English target/status) and U691 (canonical graph-paper/ink projection · exact room + furniture cell coords · authored rotation preserved · isolated bounds · NO floor slabs · NO raised wall meshes · NO player/scale figure); add U692 (preview projection suppresses player/NPC/settlement/wilderness/roads/edges/fog · wrapper passes canvas+props+bounds into `mountSlice3D()` · small + large plan camera framing · gameplay camera orientation · no generic block fallback · wired art requires a real GLB result · gameplay unchanged when preview absent). Retain U685/U687/U418/U419/U475–U479. Finish with `npm run check`.
- **done_when:** isolated authored building · canonical graph-paper + ink · real gameplay 3D camera/presentation · real GLBs at authored transforms · no world boot / settlement content · no raised geometry / fake blocks · no save mutation · LIVE screenshot proof (Builder → Preview 3D → v1.html tilt comparison; localStorage snapshot before/after = only `ie.builderPreview` changed).
- **rollback:** revert the named files (adapter + the `render3d.js` preview seam + the two page edits + tests); no engine or save touched.

## OBJECT PHYSICS ARC — placed furniture becomes physically real (8 packets, one at a time)

*Tim commission 2026-07-13: a player/NPC can lift, carry-locally, drag, push, overturn, place, throw, damage, destroy, and barricade with objects — subject to the actor's strength+size — as prose/voice tabletop adjudication (NOT mouse-drag, NOT a rigid-body sim). Engine decides outcomes; renderer only projects. Authored floorplan stays immutable initial placement; ONE canonical live-state overlay carries moved/held/damaged/obstructing/destroyed. Actor-agnostic resolver for players AND NPCs; all mutation via `effectsCore.applyDeltas()`; all randomness via `engine/rng.js`. Barricades ≠ door hardware state. Destruction leaves debris in-world; salvage is a second step. Packets: OBJ-INK-1 → OBJ-STATE-1 (absorbs FURN-PARITY-1, one WORLD_VERSION bump) → OBJ-STRENGTH-1 → OBJ-MOVE-1 → OBJ-DURABILITY-1 (= DM-GATE-1b) → OBJ-THROW-1/OBJ-BARRICADE-1 → OBJ-RUBBLE-1 (absorbs INK-WRECK-1) → OBJ-NPC-1. Each: PACKETS row → narrow failing test → smallest seam → proof ladder → STOP + report. Packets 2–8 edit `engine/**` ⇒ each needs a Form-Prompt engine-brief + Tim's OK before its engine writes.*

### OBJ-INK-1 — kill the duplicate furniture ink under a mounted GLB (3D)  ·  Phase 4 (the face) / map-fidelity  ·  **✅ LANDED v0.39.2 b157 (Basecamp, 2026-07-13; renderer-only, no engine write; packet 1 of the OBJECT PHYSICS ARC).** A furniture glyph now disappears from the 3-D ground ONLY when that exact object's real GLB mounted (`userData.glb === true`) — pure-2D and failed/unwired GLBs keep their honest ink mark; never a fabricated block (non-negotiables #13/#14/#15). SEAM: `projectBuilderDoc` mints a stable per-object id (prefers doc id/pieceId, else `fp-<i>` — the forward contract for OBJ-STATE-1's gameplay ids) carried onto BOTH the ink glyph and the prop; the shared `handDrawnInterior.drawBase(model, { suppressFurnitureIds })` skips a suppressed glyph by id WITHOUT re-indexing the seeded jitter key (surviving glyphs byte-identical, live-2D call unchanged); `render3d` reports each prop's `{id,kind,wired,glb}` and exposes `refreshGround`; the adapter, AFTER the GLBs actually mount, suppresses exactly the resolved-GLB id set and re-stamps the ground texture. Files: `public/map/handDrawnInterior.js`, `public/map/builderPreview3d.js`, `public/map/render3d.js`, `tests/U693.objInkSuppression.test.js` (NEW). Proof: U693 11/11 (pure id-correlation + source-contract) · full suite 11,141/0 · live pixel-delta (hearth 9009→872 px, bed 7684→1858 — glyph removed; crate 2304→2324 — retained) · screenshot (4 GLBs no glyph, crate ink-only, no block) · console clean · GLBs 200 · scratch key only. Rollback: revert the 3 renderer files + delete U693.

### OBJ-STATE-1 — stable objectId + canonical live-object overlay + resolver (foundation)  ·  object-physics arc / make-correct  ·  **✅ LANDED v0.40.0 b158 (Basecamp, 2026-07-13; engine, Tim-approved engine-brief v2 + P1/integrity review; WORLD_VERSION 32→33; packet 2 of the OBJECT PHYSICS ARC).** Packet 2 of the arc — the invisible foundation everything else (lift/drag/throw/smash/barricade) stands on. NO visible change, no new action; proof is identity + migration + hash. Delivers: (1) a globally-unique, deterministic, namespaced **objectId** on every Model-A furniture piece — authored `au:<structureId>:<pieceId>` at its seam, procgen `pg:<nodeId>:<slot>` at the generation seam, legacy saves by an idempotent ensureWorld backfill (`engine/objects/identity.js`); (2) a canonical **hash-visible live-object overlay** `world.objects` = { [objectId]: { placedAt?, heldByActorId? } } — empty but present on every world, added to BOTH `worldHash.js` and `worldHash.browser.js` projections; (3) a pure **`resolvedObjectPlacement(world, objectId)`** with a fully-defined return shape and `held > placed > base` precedence (`engine/objects/placement.js`) — EXISTS + tested, but NO consumer is rewired yet (that's OBJ-MOVE-1, with live proof); (4) **stable-id addressing** in `applyDeltas` modify/removeFurniture — an EXPLICIT objectId resolves ONLY by that id and NO-OPS if it doesn't resolve in the node (P1 fix per Tim's review: never fall back to index/name, which would silently mutate the wrong piece); legacy index+name ROM-4 applies only when no objectId is supplied; (5) invariants: global objectId uniqueness + valid override shape + **overlay integrity** (every `world.objects` key names a known furniture objectId; `placedAt.node` names a real node). Hash CORRECTED per Tim's review — a v32→v33 migration necessarily changes the hash (meta.version + map + world.objects all hash-visible); acceptance is deterministic-once migration + idempotent ensureWorld + save-roundtrip + replay-equality, NOT byte-identity. Files: `engine/objects/identity.js` + `placement.js` (NEW) + index.js, `engine/structures/authoredFurniture.js`, `engine/decompression/generateFurniture.js`, `engine/state.js` (v33 + `objects` default + `backfillObjectIds`), `engine/worldHash.js` + `worldHash.browser.js`, `engine/effectsCore.js`, `engine/invariants.js`, `tests/U694.objectState.test.js` (NEW, 18/18) + ~18 version-string re-pins + U454-E boot-hash re-pin (7fc17002…→7f911937…). Procgen-migration decision LOCKED IN (U694-L, not left as an untested limit): a migrated v32 procgen piece gets a compacted array ordinal, intentionally divergent from a fresh replay's generation slot (different world instances; recovering the slot would need the fragile affinity-match stable ids exist to kill) — documented in `backfillObjectIds`. Proof: U694 **18/18** (identity/backfill/resolver/failure-mode two-same-named-after-splice/**explicit-id-no-fallback modify+remove**/foreign-node no-op/legacy-compat/migration/**overlay-integrity**/read-neutrality/**divergence-lock**) · full suite **11,159/0** (determinism U19/21/22/27/30 green — replay-equality holds) · `playtest:quick` 50 runs 0 crashes · `npm run check` GREEN (convergence 135/135, screen-truth 7 goldens locked = zero visible change). Rollback: single revert → WORLD_VERSION 32, additive-only, nothing destroyed.

### OBJ-STRENGTH-1 — actor strength+size decides who can lift/carry an object (foundation + take-gate fix)  ·  object-physics arc / make-correct  ·  **✅ LANDED v0.41.0 b159 (Basecamp, 2026-07-14; engine, Tim-approved engine-brief v3 + classifier-coverage + bestiaryRef corrections; NO WORLD_VERSION bump; packet 3 of the OBJECT PHYSICS ARC).** Placed objects become physically real to *pick up*: strength + size + weight + bulk + mobility decide lift/carry/drag, for players AND NPCs AND in-combat enemies, via ONE pure resolver. Delivers: (1) **`engine/objects/mobility.js`** — one object-mobility authority `mobilityForObject(piece)` → `fixed|heavy|portable` keyed on a canonical, frozen `IDENTITY_MOBILITY` map (every live `roomDetail.FURN` key+label + `generateFurniture.TEMPLATES[].name`, each an explicit class), identity-first (a hearth/well/altar/anvil/basin is `fixed` by WHAT IT IS, not bulk), weight/bulk fallback only for unknown identities; U695 enumerates the live catalog so a NEW furniture row FAILS closed until its class is chosen; `objectPhysics(piece)` unifies weight/bulk/material/mobility. (2) **`engine/objects/capacity.js`** — pure, rng-free `actorObjectCapacity(actorFacts, objectFacts, action)` → `auto | roll | impossible` + a `difficulty` tier: `carryCap = base(MIGHT)+sizeShift`; `load = weight + bulkBurden(bulk>weight for lift/carry/throw) + actionOffset(drag/push −2, lift −1, carry 0, throw +1)`; `fixed` short-circuits impossible. (3) **`engine/objects/physicsActor.js`** — `findPhysicsActor` (party → combat enemies → present NPCs, first-wins collision), `actorSize`/`actorMight`/`actorFacts`; size stays canonical at the CONTENT source (party → `dnd.species.size`; NPC/enemy → `bestiaryRef → getMonsterDef(ref).size`), default Medium; **no `enemy.size` field ever**. (4) take-gate fix (`engine/llmPhysics.js`): the old `bulk<=2` free-take is replaced by capacity(`carry`) — a borderline take fires the existing seeded `rollPhysicsCheck` and only a passed roll mutates; the remove delta now carries the OBJ-STATE-1 `objectId` (never index-only). (5) **`bestiaryRef` preserved on combat enemies** (Tim ruling: canonical content reference, not duplicated size) — `mintEnemyFromDef`/`mintEnemyFromNpc` emit it, `ensureCombat` normalizes a bounded optional string added ONLY when set (refless/legacy enemies byte-identical → Medium), so a Cave Troll is Large *before AND during* a fight. (6) content: `size:'Large'` on the real Cave Troll catalog record (`MIGHT:20`); `TEMPLATE_NAMES` frozen export. Files: `engine/objects/mobility.js` + `capacity.js` + `physicsActor.js` (NEW) + index.js, `engine/llmPhysics.js`, `engine/decompression/generateFurniture.js`, `engine/ruleset/core/bestiary/catalog/standard.js`, `engine/combat/encounterSpawn.js`, `engine/combat/combatLifecycle.js`, `engine/state.js` (ensureCombat bestiaryRef), `tests/U695.objectStrength.test.js` (NEW, 27/27), `tests/B05.bestiaryPoolShape.test.js` (+`size` optional field). Proof: U695 **27/27** (coverage-contract fail-closed · pure exemplar matrix · bulk-affects-capacity · drag-beats-carry · findPhysicsActor order/collision/defaults · real Cave-Troll spawn→Large · all four take outcomes through `evaluatePhysicsSync` with objectId · **Section G**: ambush + NPC→combat enemies resolve Large, legacy enemy defaults Medium) · full suite **11,186/0** (determinism U19/21/22/27/30 green; U606 + 43 furniture canaries byte-identical) · `playtest:quick` 0 crashes · `npm run check` GREEN (135/135, screen goldens locked = invisible on-screen) · **live v1 proof** (weak Sera MIGHT 3 → "take the cauldron" → "…too heavy to carry", inventory unchanged — the bulk-2 cast-iron the old rule let anyone pocket). NO WORLD_VERSION bump (combat-scoped `bestiaryRef` absent from boot). Rollback: revert the 3 new `objects/` modules + take-gate swap + Cave-Troll `size` line + the 3 combat-seam `bestiaryRef` edits; additive, no migration.

### OBJ-MOVE-1 — an authored object repositions to a legal tactical cell; collision + render follow  ·  object-physics arc / make-correct  ·  **✅ LANDED v0.41.1 b160 (Basecamp, 2026-07-14; engine, Tim-approved engine-brief v4 + two render corrections; NO WORLD_VERSION bump; packet 4 of the OBJECT PHYSICS ARC — the FIRST consumer of the OBJ-STATE-1 overlay).** A player/NPC can drag/push/shove an authored piece to a new spot, and it *stays* there — the floor frees the old cell and blocks the new one, and the map draws the piece where it now sits. Authored-only (generic/procgen visible movement deferred to Model-A↔B parity). Delivers: (1) **`engine/map/spatial/tacticalPos.js`** — `authoredBaseAnchorCell` (objectId→pieceId→plan-item→anchor join, authored-only), `legalMoveTargetCell` (nearest legal free cell: in-room, indoors, excludes actor/own/occupied/reserved-door cells; held→null), `liveAuthoredBlockedCells` (ONE live occupancy projection: static furniture mask minus moved pieceIds plus their override cells — empty overlay byte-identical to the static mask), exported `reservedDoorCells` (the one canonical door-mask helper); the walk (`resolveTacticalWalk`) now reads the live projection; **FURN-1 anchor-only collision preserved** (a moved piece blocks its single new anchor cell, not a footprint); `structCellFree` signature untouched. (2) **`engine/effectsCore.js`** — the `moveObject` op writes `world.objects[objectId].placedAt = {node,structureId,room,cell,rot}` and clears held, via `applyDeltas` only, with **two HARD RULES**: it recomputes `legalMoveTargetCell` and applies ONLY when `to.cell` exactly equals it (a mismatched/forged cell no-ops), and a held object is never moved by this verb. (3) **`engine/llmPhysics.js`** — `MOVE_RE = drag|push|shove` gates the move (bare "move/slide" stay travel), runs the capacity(`drag`) gate (impossible→"can't budge"), auto→emit / roll→seeded check then emit-or-refuse. (4) **`engine/playloop.js`** — `drag|push|shove` added to `PHYSICS_VERB_RE` so the LIVE player gesture reaches the move gate (furniture-name-guarded; travel verbs unaffected). (5) **the ONE render join (`public/map/LocalMap.js` + `handDrawnInterior.js`)** — a FINALIZED authored building (gated on the durable `st.authoredPlan` marker, NOT "the floor plan has furniture") draws the ENGINE's own `f.authored===1` furniture, each piece repositioned by its live override; Model-B room-relative fractions → scene-model TOP-LEFT + LAYOUT-UNIT size at the correct cell (rect `f.w·roomW`/`f.h·roomH`; circle `r·2·roomW`; center→top-left offset); an empty authored building renders its own empty shell; procgen keeps its catalog art byte-for-byte; `floorPlanToSceneModel` passes furniture through. Files: `engine/map/spatial/tacticalPos.js`, `engine/effectsCore.js`, `engine/llmPhysics.js`, `engine/playloop.js`, `public/map/LocalMap.js`, `public/map/handDrawnInterior.js`, `tests/U696.objectMove.test.js` (NEW, 26/26). SELF-CAUGHT via the real browser: the first cut tested only the offline `evaluatePhysicsSync` gate and the router (`PHYSICS_VERB_RE`) didn't carry the move verbs, so the LIVE gesture did nothing → fixed + a Section-H live-`playerMove` test added; two render corrections followed (center-as-top-left + raw-normalized dims → proper conversion; branch-scoping so procgen catalog art isn't hijacked). Proof: U696 **26/26** (helpers · moveObject + both hard rules · target rejections incl. no-free-cell · anchor-only collision moves · byte-identical walk · verb end-to-end · **live `playerMove`** · save/load + worldHash · **render projection** center→cell base+moved · **render-branch scoping** authored/procgen-20-pieces/empty-authored) · full suite **11,212/0** · `npm run check` GREEN (135/135, screen goldens locked) · **live v1 proof** (real gesture "drag the barrel aside" → barrel base→placed (57,53), narration "You drag the barrel aside.", visible prop shifts exactly one cell up-left on the drawn canvas — centroid Δ LEFT 22.8px/UP 22.1px ≈ one cell — bed+player unchanged; a procgen tavern with 20 roomDetail pieces still renders its full catalog art). NO WORLD_VERSION bump (overlay data only). Rollback: revert the 4 engine files + 2 render files + delete U696; additive, no migration.

### OBJ-DURABILITY-1 / DM-GATE-1b — declared attacks resolve against persistent object AC, HP, and damage threshold  ·  object-physics arc / make-correct  ·  **✅ LANDED v0.42.0 b161 (Basecamp, 2026-07-14; engine, Tim-approved engine-brief rev 3 + release audit corrections; WORLD_VERSION 33→34; packet 5 of the OBJECT PHYSICS ARC).** A declared weapon attack on present furniture now resolves in the fiction as one of four outcomes — miss, absorbed, damaged, destroyed — without starting combat or exposing dice math in narration. Delivers: (1) **`engine/objects/durability.js`** — explicit physical-material profiles `{ac,maxHp,threshold}` for every live FURN/procgen/Builder identity, including kind-first correction of render-only `fire` tags, plus a safe detectable fallback; (2) **`engine/playloop.js`** — primary-target-scoped furniture selection, exact stable `objectId`, natural-1/20 d20 rule, seeded rolled weapon damage (not flat max die), threshold semantics where only damage **below** the threshold is absorbed, ordinal same-name selection with out-of-range no-op, four fiction-first prose outcomes, and clean misses that write neither durability nor Canon ruling; (3) **`engine/effectsCore.js`** — sole `damageObject` writer derives the profile from the live piece, preserves unrelated overlay fields, mirrors HP 0 to terminal `wrecked`, reconciles legacy terminal mutations to HP 0, deletes overlays atomically when furniture is removed, and ignores stale ids/caller-forged profile data; (4) **v34 state/invariants** — lazy `world.objects[id].durability = {material,ac,maxHp,hp,threshold}`, migration-safe defaults, stable hash/save replay, global identity presence, well-shaped durability, and terminal-state↔HP consistency; (5) procgen and authored objects share the same path; bare smash/break/kick remains on the legacy force resolver; rubble, typed damage immunity/resistance/vulnerability, throwing/barricading, and NPC parity remain later packets. Files: `engine/objects/durability.js` + index, `engine/playloop.js`, `engine/effectsCore.js`, `engine/state.js`, `engine/invariants.js`, furniture material census exports, version pins, `tests/U697.objectDurability.test.js` (NEW). Release audit caught and fixed three pre-commit truth bugs: second-object targeting counted furniture named outside the primary clause / clamped invalid ordinals, picking up a damaged object left an orphan overlay, and threshold-equal damage was incorrectly absorbed. Proof: U697 **27/27** + adjacent U371/U643/U651/U694 **32/32** · `npm run check` GREEN (**11,239/0**, determinism U19/U21/U22/U27/U30 included, convergence **135/135**, position probe clean, screen truth **7/7** goldens locked) · live `v1.html` Mira/Longsword vs stone basin produced all four visible outcomes over one persistent arc: MISS → ABSORBED → DAMAGED → DESTROYED (no browser errors; old-save migration warnings only). Rollback: revert this commit → WORLD_VERSION 33; durability is additive overlay data and terminal wreckage remains represented by the pre-existing Model-A state vocabulary.

### OBJ-HOLD-6A — held identity + honest release + ONE room truth  ·  object-physics arc / make-correct  ·  **✅ LANDED v0.43.0 b162 (Basecamp, 2026-07-15; engine, Tim-approved engine-brief rev 3; NO WORLD_VERSION bump; packet 6 of the OBJECT PHYSICS ARC).** Picking up a supported object stopped deleting it and minting a fresh generic pack item — it now rides in the actor's arms as the SAME canonical piece, identity and any accrued durability damage intact, and every seam that used to lie about it (presence, room lists, egress) now tells the truth. Delivers: (1) **`engine/effectsCore.js`** — two new writers, `holdObject` and `placeObject`, each re-deriving its full precondition set from scratch (live piece · supported = authored+objectId+grounded · not wrecked · mobility≠fixed · actor resolves and is co-located via the LIVE room · not already held by another · hands free) so a forged delta can never steal, lift masonry, or take through a wall; release destinations are ALWAYS engine-derived (`legalPlaceTargetCell`) — a caller-supplied cell is ignored outright. (2) **`engine/invariants.js`** — the two one-held laws: held⊥placed (a record can't be both) and one-actor-one-object (uniqueness across all overlay records); deliberately NO actor-existence law (a vanished holder must never brick an unrelated save — OBJ-NPC-1's problem). (3) **`engine/map/spatial/tacticalPos.js`** — `legalPlaceTargetCell` (nearest free cell from the actor or a named grounded floor object in the SAME live room) plus held-object exclusion in the live occupancy projection (a held object blocks nothing, is drawn nowhere). (4) **`engine/llmPhysics.js`** — `heldObjectOf` (the one canonical "what's in this actor's arms" helper, used everywhere below), held-object detection/release-verb routing, and the supported-domain branch of the take gate (capacity-gated exactly like the legacy take, but calls `holdObject` instead of remove+createItem). (5) **`engine/playloop.js`** — the carry-lock (`carryLockRefusal`, shared by door exit / window exit / both interior travel bridges): egress refuses fiction-first while holding, narrating why instead of letting a silent no-op read as success; presence and where-is questions answer "you're carrying it" BEFORE the floor-absence branch can call it missing; a swing at the object in your own arms is guidance, never a roll. (6) **`engine/structures/interiors.js`** — the structural backstop: `exitStructureInterior` itself no-ops while any object is held, so a future call site that forgets the carry-lock still cannot leak a held object outside. (7) **`engine/structures/roomObjects.js`** + **`engine/grace/gracefulAdjudication.js`** + **`public/map/LocalMap.js`** — `objectsHere`/room-object answers/the floor render all resolve a supported piece's room membership by IDENTITY through `resolvedObjectPlacement` (held → no floor list in any branch; placed → its live room ONLY), replacing the old name-keyed assignment map that would leave a dropped object mechanically stuck in its original room. (8) **`public/v1.js`** — a new "In your arms" inventory row, deliberately separate from pack categories (you're carrying it, not pocketing it). Unsupported (procgen/generic/ungrounded) pieces keep today's remove+createItem path byte-identically — this packet is scoped to the supported domain; FURN-PARITY-1 owns the rest. Files: the 8 above + `tests/U698.objectHold.test.js` (NEW, 47/47 across sections A–H: domain gate, the one-held rule, writer trust boundary, the 8-step cross-room contract, presence/visibility, the carry-lock, honest release, determinism/replay). Proof: U698 **47/47** · full suite **11,286/0** (determinism U19/21/22/27/30 included) · `npm run check` GREEN (convergence 135/135, screen truth 7/7 goldens locked) · live `v1.html` receipt (seeded `loaderDemo2`, MIGHT 20 Bran beside the authored cooking pot): "take the cooking pot" → "You take up the cooking pot — it's in your arms now." + a new "IN YOUR ARMS" panel · "is there a cooking pot here?" while holding → "You're carrying the cooking pot — it's right there in your arms." (previously would have said no) · "go outside" while holding → refused fiction-first, world unchanged · "set the cooking pot down" → released, panel clears · presence question afterward → flips back to the exact original floor answer · "go outside" with empty hands → succeeds, map zooms out to the settlement; console clean throughout. NO WORLD_VERSION bump (overlay data only — `world.objects[id].heldByActorId`/`placedAt` already existed from OBJ-STATE-1/OBJ-MOVE-1). Rollback: revert the 8 files + delete U698; additive, no migration.

### OBJ-THROW-6B — engine-owned throw: reach, landing kinds, two-sided impact  ·  object-physics arc / make-correct  ·  **✅ LANDED (commit `857d8fa1`, 2026-07-16; U699 — ROW WRITTEN RETROACTIVELY 2026-07-18 by the OBJ-BARRICADE-6C window, which found the code shipped with no PACKETS row, no changelog entry, and no version bump of its own; it rode between b163 and b164).** A throw of a supported object stopped falling through to the generic freeform floor (where it was adjudicated as a WITS check while nothing moved) and became real physics: `engine/objects/throwing.js` (the pure layer — verb grammar, phrase split, the `MAX_THROW_CELLS`/`THROW_NOISE` product constants, the hard-surface predicate, the two-sided impact filter), `resolveObjectThrow` in playloop (preflight → seeded accuracy vs the target's AC → damage both ways → landing), an EPHEMERAL `ref.mode:'throw'` on the placeObject writer selecting the 6B landing kinds (`room`|`wall`|`object`) with writer-owned reach, and the reciprocal-damage rule (a projectile only takes damage back off a HARD surface; `unknown` fails closed). Accuracy/impact reuse the existing IMPROVISED-WEAPON contract (d4 + MIGHT, no proficiency) rather than inventing a scale. Files: `engine/objects/throwing.js` (NEW), `engine/objects/index.js`, `engine/map/spatial/tacticalPos.js`, `engine/effectsCore.js`, `engine/playloop.js`, `tests/U699.objectThrow.test.js` (NEW). Re-verified green 2026-07-18 (U699 in the 11,527/0 suite).

### OBJ-BARRICADE-6C — wedge an object against a door, and the door honours it  ·  object-physics arc / make-correct  ·  **✅ LANDED v0.51.0 b172 (Basecamp, 2026-07-18; engine, Tim-approved Form-Prompt brief; NO WORLD_VERSION bump; closes packet 6 of the OBJECT PHYSICS ARC).** The last physical verb in Tim's commission with no engine behind it. Shoving a supported object against a door now records `world.objects[id].obstructs = { structureId, doorId }` beside its `placedAt` cell, and every traversal seam honours it. **Seams:** (1) **`engine/objects/barricade.js`** (NEW, pure leaf, throwing.js import law) — two phrase shapes (`<verb> the DOOR with the OBJECT` · `<verb> the OBJECT against/across/in front of the DOOR`), the family boundaries (throw keeps throw/hurl/toss/fling; ram/bash/kick stays barrier-force; "throw up a barricade" stays the build lane), and the product constants `BARRICADE_ACTION='drag'` / `MIN_BARRICADE_BULK=2` / `BARRICADE_NOISE=1`. The drag action is a **reversibility guarantee, not tuning**: what you could shove there you can always shove away, so no actor can strand themselves. (2) **`engine/effectsCore.js` `barricadeObject`** — the sole `obstructs` writer, re-deriving every precondition (live piece · supported · not wrecked · qualifies · not another's arms · actor's OWN pos inside this structure · the door is a real record of this structure adjacent to the actor's room · object in the actor's LIVE room · one door one barricade); the cell is ALWAYS engine-derived and a caller-supplied cell is ignored. `moveObject`/`holdObject`/`placeObject` each delete `obstructs` — the obstruction is a claim about where the object stands, so it cannot survive the object moving. (3) **`legalBarricadeTargetCell`** (tacticalPos) — the ONE derivation that deliberately targets a door approach cell, and the packet's whole meaning: furniture can never ACCIDENTALLY seal a doorway (move/place still refuse the reserved set, U696/U698 untouched) but a body can DELIBERATELY wedge one there. Derived from GEOMETRY, never the plan door's `dir` compass label (they disagree on the live fixture — U725-G2 locks it). (4) **`engine/structures/interiors.js`** — `interiorDoorBlock` reports `barricade` ALONGSIDE the hardware (`state`/`crossable`/`needsForcing` byte-unchanged: an OPEN door with a wardrobe against it does not admit you, and unbarring it does not move the wardrobe); `moveWithinInterior` and `exitStructureInterior` are the structural backstops, the latter with a `{ via:'window' }` exemption because a barricade is about one DOOR, not about the body (that distinction is the carry-lock's). (5) **`engine/playloop.js`** — detection + `resolveObjectBarricade` (DM-style asks for which-door / with-what rather than choosing a piece of the world on the player's behalf), the three door-egress refusals beside the carry-lock, the interior-move refusal, and `facingBarricadedDoor` so a force declaration at a barricaded door NEVER fakes success. (6) **`engine/invariants.js`** — obstructs is well-shaped, implies a placement, excludes being held, one door one barricade; deliberately NO door-existence law (same reasoning as the absent actor-existence law). Tests `tests/U725.objectBarricade.test.js` (NEW, 27/27, sections A–G). Proof: suite **11,527/0** · `npm run check` GREEN (convergence 135/135, position probe clean, screen truth 7/7 goldens locked) · `playtest:quick` 50 runs 0 crashes · **live receipt** (seeded loaderDemo2, Bran MIGHT 20, East Scullery): "shove the cooking pot against the door" → *"You drag the cooking pot across and wedge it against the doorway…"* → "go to the hearth room" → refused, player did not move → "kick the door down" → *"…it shifts an inch — then stops dead. The cooking pot you wedged behind it takes the whole blow"* (no fake success, world untouched) → "drag the cooking pot aside" → cell 65,52→65,51 and `obstructs` cleared → "go to the hearth room" → *"You step through into the hearth room."* **Scope correction made in flight and reported:** WINDOWS were promised in the brief and are NOT shipped — doors carry a canonical stable id (doors.js `interiorDoorId`/`exteriorDoorId`), a canonical state record, and an existing traversal gate; windows have none of the three, so including them meant inventing an id namespace and a fourth gate. Deferred honestly rather than shipped weak. **Honest boundary:** enemies never move (`moveCombatant` is player-only — DEATH-TRUTH-1c), so a barricade blocks the PLAYER's traversal and is narrated truthfully; it does not yet stop a foe the engine never walks through doors. Rollback: revert the commit; `obstructs` is additive overlay data, no migration either way.

### RULING-FX-1 CORRECTION — an EMPTY fx proposal silently erased real physical deltas  ·  make-correct / live-caught  ·  **✅ FIXED v0.51.0 b172 (Basecamp, 2026-07-18, caught on the live build while verifying OBJ-BARRICADE-6C's own advertised affordance).** Shipped in b171 and live for one day. **Repro:** barricade a door, then "drag the cooking pot aside" — the prose said *"It moves to the side"*, and the pot did not move; the next move was still refused. **Root:** `/api/physics-fx` returned `{ plausible: true, deltas: [], result: "…" }`; `groundFxProposal` accepted it (`validateDeltas([])` is ok) and the consumption seam substitutes BOTH halves (`physics.deltas = rulingFx.deltas`), so the empty list wiped the lane's real `moveObject` delta while keeping prose asserting the move — narration claiming a world change canon does not have. **Systematic, not a one-off:** the sealed fx vocabulary is deliberately object-only (`modifyFurniture|createItem|removeItem|removeFurniture`) because the engine owns position, so a CORRECTLY-behaving model returns `[]` for every positional action — and every positional action on the offline physics lane lost its effect. **Fix:** one line + rationale in `groundFxProposal` — a proposal with zero deltas is never consumed (it has nothing to ground, so it is not a ruling), restoring the deterministic template deltas AND sentence, i.e. exactly the pre-RULING-FX-1 fallback a rejected proposal already took (U723-C1). Regression lock: `U725-D5`. Re-verified LIVE after the fix: the drag really moves the pot and the way opens. U720–U724 27/27 unchanged.

### FINDING (filed, not fixed) — `reservedDoorCells` reserves the wrong cells when a door's compass label disagrees with its drawn geometry  ·  soft-lock guard / queued  ·  **QUEUED 2026-07-18 (found by OBJ-BARRICADE-6C; locked as a characterization test, `U725-G1b`).** `reservedDoorCells` (tacticalPos) derives a doorway's two approach cells from the plan door's `dir` (north/south → y±1, east/west → x±1). On the loaderDemo2 fixture the hall→boot-room door carries `dir:'south'` while the two rooms sit SIDE BY SIDE, so the reserved pair lands in the WALL BAND and the real crossing is left unreserved — i.e. the guard that exists to stop furniture soft-locking a building is not guarding that doorway at all. OBJ-BARRICADE-6C is unaffected (it derives its cell geometrically and refuses traversal from the barricade RECORD, not the mask). NOT fixed here because the repair changes which cells ordinary furniture may occupy, which moves placement and pathing for every world — a WORLD_VERSION-shaped packet with its own hash/golden re-pinning. `U725-G1b` asserts the CURRENT (wrong) behaviour so the fix has a red-first target.

### COOK-1 — the smallest REAL cooking affordance (decide before wiring)  ·  texture / gear seam  ·  **QUEUED 2026-07-09 (FUNC-MINIS-1 follow-up; Tim: queue, do not start — "decide the smallest real cooking affordance before wiring it").** The placed cookpot is engine-real (iron, container, dentable — b144) but NO cooking mechanic exists to join; wiring one would be invented design. Packet = DESIGN FIRST: bring Tim 2–3 smallest-cut options (e.g. cooking rations at an intact cookpot + fire upgrades the consumable’s rest value · a cooked `meal` item via the existing P-71 crafting lane · a flavor-only breather upgrade), with costs/consequences, THEN wire the chosen one. Constraints: deterministic, no new economy, kind-gated via the FUNC-MINIS contract (kind ‘cookpot’ + intact predicate, authoredFurniture.js), narrate-the-read-never-the-number.

### FURN-PARITY-1 — procgen furniture tracks destruction like placed furniture  ·  make-correct  ·  **IDENTITY HALF ABSORBED by OBJ-STATE-1 (2026-07-13); destruction-join generalization still QUEUED.** OBJ-STATE-1 gave procgen pieces a stable `pg:<nodeId>:<slot>` objectId (the identity FURN-PARITY-1 needed) — the remaining work is generalizing the Model-A↔Model-B destroyed-set join off that objectId so procgen cover/blocking can't go stale after destruction. Pre-existing gap FUNC-MINIS-1 surfaced and scoped OUT: a GENERIC (roomDetail role-loadout) piece smashed via Model A joins back to its Model B twin only by AFFINITY guess, so procgen cover/blocking can go stale after destruction (authored pieces join exactly by pieceId — b144 fixed those). Path: give procgen loadout pieces the same stable identity at their seeding seam (mint pieceIds for roomDetail items + carry provenance on generic Model A pieces, mirroring seedAuthoredNodeFurniture), then reuse the SAME destroyed-set join (authoredFurniture.destroyedAuthoredPieceIds generalizes). WATCH: byte-identity/worldHash for untouched worlds, ROM-4 name-keyed ops, placement tests U545/U546.

### INK-WRECK-1 — wrecked furniture RENDERS wrecked (2D ink live-state overlay)  ·  Phase 4 (the face) / map-fidelity  ·  **✅ LANDED v0.45.0 b164 (2026-07-16, Basecamp landing of the completed dirty-tree work; U703 7/7; suite 11,412/0; check GREEN ×2).** Shipped as ONE suppression treatment (destroyed ⇒ draws nothing, both projections — LocalMap buildAuthoredSceneFurniture AND placeFromNode flattenRoomFurniture — off the SAME destroyedAuthoredPieceIds authority; U703's header records why the two-treatment plan was deliberately not taken: the authority unions absent+wrecked and no engine consumer distinguishes them). The rubble upgrade = OBJ-RUBBLE-1, in flight below. Original row (2026-07-09): The plan ink still draws a destroyed piece intact (b144: barrel gone from world truth + cover + blocking, still inked on the local map). Live-state source is ready-made (authoredFurniture.isFurnitureDestroyed / destroyedAuthoredPieceIds); the renderer’s furniture draw (LocalMap / floorPlan consumers) needs the overlay: ABSENT ⇒ debris mark or nothing; WRECKED ⇒ greyed/broken glyph. Map-fidelity law applies (a map fix isn’t DONE until it registers on the player map, verified live). Renderer lane — parallel-safe; pairs naturally with FURN-PARITY-1 but must not wait on it (authored pieces alone are worth the ink truth).

### WIN-LOOK-1 — looking through/at a window answers from the window, not a room survey  ·  make-correct / DM_TEST pillar  ·  **✅ LANDED v0.35.5 b149 (Basecamp/Fable main-checkout, Tim-authorized in-chat brief 2026-07-10; diagnosis-first, revised twice mid-brief on Tim's own read of a fresh red test).** Diagnosis (empirical, not read-only guesswork): calling `playerMove()` directly does NOT hit playloop.js's own internal meta-gate — that gate already carves out `bareLookAround` for any `META_LOCATION`-shaped text, so the turn falls through inside playloop.js on its own. The remaining bug was two-layer: **(1)** `windowVerbKind`'s shutter-open/close verb+object regex required EXACT adjacency between article and noun — "open **the shuttered** window" broke on the adjective, falling the whole declared action to the generic trivial-action floor (confirmed: pre-fix, direct `playerMove()` on the verbatim gate text returned "You do so without difficulty."); **(2)** even with adjacency fixed, a compound "open the window **to look** outside — what do I see?" needs BOTH halves resolved as one action (open, then answer the view) — the single-verb-kind dispatch would only ever return one of them. **Root A** (adjective tolerance: `shuttered/closed/barred/locked` between verb and noun) + **Root B** (compound open-then-look, checked before the plain LOOK/shutter-open branches, chains the existing shutter-open mutation + `windowView()` in one turn) — both entirely inside `engine/playloop.js`. **Outer-gate guard** (added after Tim caught a scope drift mid-brief): new exported `detectWindowActionIntent(world, text)` mirrors DM-GATE-1a's `detectObjectAttackIntent` exactly, guarding `public/v1.js:745` and `scripts/dm-playtest.mjs`'s pre-`playerMove` meta gate — without it, the LIVE game and the Opus-gate harness still swallow the turn as a room survey even though playloop.js's own dispatch was already fixed (proven via a harness-mirrored test, not assumed). `gracefulAdjudication.js` and `roomWindows.js` untouched, as scoped — `windowView`'s LOS/fog logic was proven correct in diagnosis and needed no changes. Tests U684 (9 asserts, red-first — confirmed red via `git stash` on the implementation files, not just written-then-passing); suite 11085/0; check GREEN; `worldHash` ×2 replay identical after a shutter-open mutation. Verbatim gate turn (`docs/playtests/gate-runs/gate-2026-07-10T14-53-34-501Z-v1.jsonl` t3) now opens the shutters AND answers a grounded, fog-honored view via the harness-mirrored router. **Deliberately NOT fixed here (Tim's explicit scope call):** t4's "Wait, you didn't answer…" phrasing (a different, un-adjacent word-collision) — U684 documents it as an open finding, not silently patched.

### INV-CONT-1 — inventory-possession continuity questions reconcile from item history, not a generic dodge  ·  make-correct / DM_TEST pillar  ·  **QUEUED 2026-07-10 (07-10 re-gate finding; Tim: queue, do not start).** "Hang on, when did it go into my pack? I thought I was holding it." → the generic no-info sink ("the moment simply doesn't answer" — Confused newbie, verbatim gate turn), mechanics tag `[info-check → no-record...]` but DIFFERENT wording than DM-GATE-1e's R3 grounded-decline fix. SAME shape as DM-GATE-1d's B3 (a continuity challenge dodged/retconned instead of reconciled from canon) but a DIFFERENT fact domain: B3's `tryContinuityReconcile` (playloop.js) is scoped to FURNITURE pieces via `detectPhysicalInteraction` — an inventory item transitioning hand→pack has no furniture match, so B3 correctly never fires here. Needs: a party-inventory-scoped sibling of B3's reconcile pattern — read the timeline/event history for the item's actual pickup/equip/stow moment and cite it (warm case), or concede honestly when the engine's own state genuinely has no such event recorded (cold case) — mirroring B3's warm/cold split exactly, never re-inventing a false history. Likely files: engine/playloop.js (new sibling near tryContinuityReconcile), possibly engine/effectsCore.js item-move delta history if no queryable record currently exists (read-only investigation first — do not assume the data is there).

### LETTER-REF-1 — pronouns/entities inside authored letter prose ground to the letter, not the roster  ·  make-correct / DM_TEST pillar  ·  **QUEUED 2026-07-10 (07-10 re-gate finding; Tim: queue, do not start).** "Who's 'they' — which two people got married, and who's the one holding a grudge?" → a present roster NPC's name substituted as the answer ("Elske Nightherd, a representative — one of the folk here, watching from nearby" — Lore-hound, verbatim gate turn), confirmed via mechanics-tag forensics NOT DM-GATE-1e's R2 multi-identity fix (R2 requires ≥2 roster names literally in the question text; this question names zero — it's asking about characters described only inside the letter's own prose, generateFurniture.js's LETTER_BODIES pool). Root: the letter bodies are flavor STRINGS with no structured "who is mentioned in this letter" data — nothing for any resolver to query, so the fallback grounded-outcome floor substitutes a present NPC as a plausible-sounding but wrong answer. Needs DESIGN FIRST (a content/grounding gap, not a pure routing fix): decide the smallest structured shape for "this letter names person/relationship X" (a lightweight tag on the LETTER_BODIES pool entries themselves is the likely smallest cut) before wiring an answerer. Likely files: engine/decompression/generateFurniture.js (LETTER_BODIES + containerItemText), engine/playloop.js (a new grounded-answer branch reading the tag). Mine docs/biblioteca/ for prior art on structured-content-in-flavor-text before scoping (per the repo's demand-pulled research rule).

### ROLL-COMP-1 — composer roll/narration numeric mismatch (mechanics and prose must agree)  ·  make-correct pillar  ·  **QUEUED 2026-07-10 (07-10 re-gate finding; Tim: queue, do not start).** "I bar the cottage door from outside and shout that there's a fire, watch who comes running." → mechanics line reads `[roll:5 vs DC:12 → failure | margin:-7 | approach:insight | stake:time | risk:0.48 | stat:WITS-2]`, but the composed narration says "your shadow looks slightly wrong (18)—it doesn't land, but you learn" — quoting **18** where the roll was **5**, and reading as a near-miss/partial ("doesn't land, but you learn") where the mechanics say outright **failure**. UNRELATED to DM-GATE-1d/1e — a composer-layer numeric desync (composer.js narration assembly), not a playloop routing/detection issue; first occurrence of this exact shape in the gate ledger (CRUNCH_INCONSISTENCY class). Needs diagnosis-first: find where the composer's flavor-text picker draws its "quoted number" independent of the actual roll result, and where outcome-tier wording (failure vs mixed) can drift from the resolver's own outcome tag. Likely files: engine/composer.js, engine/resolve.js (outcome→prose mapping — read-only first, do not assume which layer owns the drift).

### GATE 2026-07-10 — post-1e re-gate (v0.35.2 b146 · **5/48, was 6/48 (07-09) / 11/48 (07-07 baseline)** · `opus-gate-2026-07-10.md` · measure-only per Tim.** Two prior attempts today FAILED before reaching a report (network fetch failure mid-run, then API-credit exhaustion mid-run — Tim topped the key to $19.96, third attempt completed clean). **CLASSIFICATION (mechanics-string forensics against the audit JSONL, not just narration pattern-matching): ZERO of the 5 are old 07-07-cluster survivors, ZERO are 1e's own 6 survivors recurring, ZERO are regressions caused by 1d/1e.** All five are NEW/adjacent residuals in code paths 1d/1e never touched: (a)×2 **window look-and-report** — "open the shuttered window, what do I see outside?" swallowed by the room survey; SAME shape as B1 (object-look vs META_LOCATION) but B1 was deliberately carried-item-scoped (`carriedItemLookReferent` only scans inventory) — a window is present furniture, never matched, never should have; (b) **letter's internal characters unresolvable** — "who's 'they' — which two got married?" (asking about the LETTER's prose content) got a present-NPC name substituted; confirmed NOT my R2 multi-identity fix (empty mechanics ≠ R2's explicit `[person → grounded | identity×N]` tag; R2 requires ≥2 roster names literally in the text, this question names zero) — a content-comprehension/grounding gap, not a routing gap; (c) **item-possession continuity** — "when did it go into my pack?" hit the generic no-info sink (confirmed via mechanics tag) with DIFFERENT wording than R3's fix; SAME shape as B3 (continuity dodge) but B3 is furniture-piece-scoped (`detectPhysicalInteraction`) and an inventory transition has no furniture match; (d) **composer roll/narration desync** (CRUNCH) — mechanics say `roll:5 vs DC:12 → failure`, narration says "(18)…it doesn't land, but you learn" — a numeric mismatch in composer.js, unrelated to any playloop routing lane. 0 new failure MODES (Chao1 CI [5,5]); 2 judge errors marked/legacy-passed. Findings NOT queued as a packet — awaiting Tim's call on whether/how to bundle (three distinct shapes: object-look-family, continuity-dodge-family, composer-numeric-bug). Ledger reconciled: Tim topped key to $19.96 → −$0.60 (two failed attempts, estimated) → −$0.96 (this run) → **$18.40 balance**.

### DM-GATE-1e — the re-gate's five survivors  ·  make-correct / DM_TEST pillar  ·  **✅ LANDED v0.35.2 b146 (Basecamp/Fable main-checkout, Tim-authorized in-chat brief 2026-07-09; R5b kept in per his ruling).** All five roots red-first on verbatim re-gate inputs, fixed in playloop ONLY: **R1** attribute asks answer the real sheet ("Your sheet, plainly: Might 6…"; Might/Agility/Wits/Charm/Grit joined the referent stopwords) · **R2** multi-name who-are asks answer identity per roster NPC · **R3** record-keeping questions skip the presence branch and decline by naming the RECORD-KIND (Tim's wording rule; "The room is quiet" never answers) · **R4** presence-about-HERE questions never bridge/egress (the TRAVEL BRIDGE was the true owner) while presence-about-ELSEWHERE deliberately keeps the U319/LH-2 egress-door law — a first-cut overreach the FULL ladder caught and corrected · **R5a** "lie down" is posture, never social:deceive · **R5b** lying on a burning piece wounds through the existing instant-wound machinery (2 HP, no roll, state-gated; cold pallet harmless). Tests U671–U676 incl. worldHash ×2 on all five; suite 11,046/0; check GREEN; live receipts (stats digits match the panel; burn lands 11/13 in prose + panel, screenshot). **NEW QUEUE ROW → ENV-FIRE-1** (systemic fire: spread, per-turn burn aura, NPC arson response — worldTick/env design space, split per the brief). Remaining from the gate series: DM-GATE-1b only (separate, standing ruling).

### ENV-FIRE-1 — systemic fire hazard (spread · per-turn burn · the world reacts)  ·  env/worldTick design space  ·  **QUEUED 2026-07-09 (split from DM-GATE-1e per Tim's ruling: "the systemic fire simulation belongs in ENV-FIRE-1").** DM-GATE-1e closed deliberate CONTACT with a burning piece (2 HP, no roll, engine/playloop tryBurningContact). Still open, deliberately: standing NEXT to fire over turns (a burn aura / hazard tick), fire SPREADING piece-to-piece and room-to-room over worldTick, smoke, NPCs reacting to arson in progress (flee/bucket-brigade/witness deeds — the chaos persona's fire drew no crowd response), and burning structures degrading. Design-first: this is a simulation loop (worldTick/env seam), not a router fix; determinism + worldHash discipline apply; MP-arc hooks (arson as a witnessed deed) should ride the existing recordDeed chokepoint. Mine biblioteca precedent before scoping.

### GATE 2026-07-09 — post-1d re-gate (v0.35.1 b145 · **6/48, was 11/48** · `opus-gate-2026-07-09.md` · $0.96, balance $12.38)  ·  measure-only per Tim  ·  **THE 07-07 CLUSTER IS CLEAR: all 11 baseline failures gone — zero recurrence of the object-attack lane, the modifier leak, the mirror shard, the chest egress, the lantern retcon, the letter authorship (the newbie asked "Who wrote this to me?" VERBATIM and it passed), and the villager cut-down. Three of four baseline bug classes at zero (COMBAT_NOT_STARTED · DM_ARTIFACT_LEAK · CANON_HALLUCINATION); 0 NEW failure modes (Chao1 stable, CI [5,5]); 1 judge error (marked, legacy-pass).** Six survivors = five roots, ALL deterministic-engine (no INT-translator involvement — the harness router has no /api/intent by design): (a) ×2 the SHEET-STATS ask loses to person-clarify ("Give me numbers for Might, Agility…" → "I haven't introduced anyone named Agility") — the sheet lane must beat the referent lane on attribute nouns; (b) "who ARE Elske/Dalla/Asha" answered only positionally ("none are in your bedchamber"), identities never delivered though canon has them; (c) a pointed record-keeping question still dies in the "The room is quiet" no-info voice (AG-4 residue class — B4 fixed the artifact-grounded case only); (d) presence+move compound ("anyone else around? Maybe I should head to the front") → egress + "no such place hereabouts" (B2 fixed OBJECT destinations; "the front" is a place-ref — the presence half must win or the move must resolve honestly); (e) lying on a FLAMING pallet = zero damage (fire-hazard consequence gap, CRUNCH_INCONSISTENCY). Findings await Tim's call — candidate DM-GATE-1e cluster; NOT started per instruction.

### DM-GATE-1d — Cluster B close: the five non-object gate leftovers  ·  make-correct / DM_TEST pillar  ·  **✅ LANDED v0.35.1 b145 (Basecamp/Fable main-checkout, Tim-authorized in-chat brief 2026-07-09; B5 kept in per his ruling).** All five re-proven still-red on b144 (mirrored router, verbatim gate inputs), then fixed red-first in their named lanes: **B1** mirror-shard look answers from the item's authored notes (grace object-referent guard; isMetaQuestion untouched) · **B2** walk-to-present-object is CONTACT, never egress/travel (the "walk to the iron-bound chest" turn used to teleport out+in via [egress:repair]; now "You cross to the chest… Inside: a folded letter") · **B3** continuity challenges reconcile from canon (timeline cite / honest concession; NEVER re-execute, never "has always burned") · **B4** letter authorship answers from the artifact ("never signed" — the authored pool is unsigned) · **B5** "cut down the first villager" = real combat vs a present roster NPC (cut/strike-down joined the violence regexes; fuzzyMatchNpc's generic-person arm resolves) AND the cruelty deed records at DECLARATION with witnesses (villager/townsfolk/bystander joined tryDarkDeed's civilian class — the constitution is not dodgeable by word choice; named fair-fight parity pinned). Tests U665–U670 (incl. worldHash ×2 on all five sequences); suite 11,023/0; check GREEN; live receipts (no egress live · COMBAT HUD vs Elske Nightherd, screenshot). HONEST NOTE: live typed text rides the INT-arc translator, which can rephrase a tail (live chest turn = examine-at-contact; the deterministic floor does full open+reveal — INT re-benchmark flag, not a 1d regression). **Remaining from DM-GATE-1: 1b only** (persistent object AC/HP, own WORLD_VERSION — Tim's standing ruling: separate). Re-gate now worth running.

### DM-GATE-1 — the Rules-Lawyer combat/object dead-ends (Opus gate 2026-07-07, b135)  ·  make-correct / DM_TEST pillar  ·  **QUEUED from the gate (11/48, 23%; report `docs/playtests/opus-gate-2026-07-07.md`).** 0 NEW failure modes (Chao1 stable at 5) — the regression from 4/48 is ONE adversarial Rules-Lawyer session hammering known-weak mechanical paths (other 3 personas = 4/36 ≈ 11%). NOT from today's Carl/map/house-builder work (those are aldermere + renderer; the gate runs tallow, and the failures are in combat/object resolution). The concrete cluster to fix: (a) **"attack the iron-bound chest"** → no combat vs an OBJECT target, no roll, action ignored (COMBAT_NOT_STARTED + DM_TEST_DEADEND); (b) **attack rolls bounced back as UI prompts** ("Roll d20 and add that", "For damage: the Worn Blade rolls 1d6") instead of resolved in fiction — the hide-the-math law inverted; (c) **modifier-breakpoint table dumped** to the player (DM_ARTIFACT_LEAK); (d) **continuity challenges** (the unlit-lantern question) RETCONNED/gaslit ("has always burned") instead of reconciled (CANON_HALLUCINATION); (e) an **ungrounded item the player invents** ("the Mirror shard") dead-ends into room re-description. Root theme = the DM has no clean path for "attack/interact with a non-NPC object" and leaks/bounces the crunch. Needs a diagnosis-first packet (some may want Tim's design input on how much crunch to surface). Audit: `gate-runs/gate-2026-07-07T17-03-36-030Z-v1.jsonl`.
- **✅ DIAGNOSIS LANDED + SPLIT ACCEPTED (Tim, 2026-07-07) — diagnosis only, implementation NOT green-lit → `docs/briefs/DM-GATE-1-diagnosis.md`** (kept untracked by Tim's call; source-backed, all 11 failures reproduced deterministically LLM-off, routes matched the gate JSONL byte-for-byte). **ROOT (proven):** no object-damage lane — attack verbs (`attack/hit/slash/stab/hack/cut`) match neither the NPC-combat detectors (bail on inanimate targets by design, `playloop.js:10354`) nor the FORCE path (`PHYSICS_VERB_RE`/`FORCE_VERB_RE` omit attack verbs); **and** the authoritative router is the OUTER meta gate (`public/v1.js:743` + harness `dm-playtest.mjs`), which lacks the "declared action beats a meta query" guard `playloop.js:1342` has — so an attack carrying a rules question is answered as raw mechanics before `playerMove` runs (`playloop.js:1342`'s guard is shadowed; confirmed to swallow NPC-attacks-with-questions too). Cluster A (the core: RL t7/t8/t9/t11) is object-attack routing + the meta guard; Cluster B (mirror flavor-item, "walk to chest" movement grab, lantern continuity-reconcile, "who wrote this" answerability, villager NPC-combat-eligibility) = separate follow-up packets, not this one. **SPLIT (Tim's ruling):** **DM-GATE-1a** = object attack *routing + damage-state resolution* via the EXISTING hardness/state path — **adds NO persistent object AC/HP** (honesty guardrail: never call it "object HP"); **DM-GATE-1b** = real persistent object AC/HP/material thresholds (D&D contract, own WORLD_VERSION bump, later). Tests U643–U649 specced (U640–642 taken).
- **✅ DM-GATE-1a LANDED v0.33.3 b136 (Tim-authorized engine-brief `docs/briefs/DM-GATE-1a-form-prompt.md`; Homebase main-checkout, committed local, NOT pushed). Tests U643–U649 (13 assertions) red→green; full suite 10937/0; `npm run check` GREEN (convergence 135/135, determinism, screen-truth); LIVE v1.html receipt (Bryn, aldermere Bedchamber): "I attack the straw pallet with my Worn Blade. What's my attack roll, and does it take damage?" → _"Your Worn Blade catches the cloth, and the straw pallet tears open."_ [roll:13 vs DC:8 → success | approach:force | stake:action] — no meta swallow, no capability lecture, no "Roll d20", no combat, no player wound, weapon+material-aware.** The fix: new exported `detectObjectAttackIntent` (attack-verb ∧ furniture name-match ∧ no present NPC); guards on all THREE meta-gate sites (`v1.js:743`, `dm-playtest.mjs`, `playloop.js` internal) + the two `answerCapability` rules-question branches + the take-the-weapon `TAKE_THEN_ACTION_RE`; attack verbs normalized to `break` for the physics eval so the swing yields graded damage-state via the EXISTING hardness/`rollPhysicsCheck` path; weapon+material-aware narration. NO persistent HP/AC, NO WORLD_VERSION bump. **HONEST SCOPE NOTE:** fixes RL t7/t8/t11 (declared object attacks) + the latent no-question form. **RL t9 NOT fixed** — it declares no object attack ("what modifier am I adding? Roll the 1d6"); its breakpoint-table dump is a meta-answer-quality leak living in `gracefulAdjudication.js` (forbidden this packet) → **Cluster B / DM-GATE-1c follow-up.** **DM-GATE-1b** (persistent object AC/HP/material thresholds, own WORLD_VERSION) still queued, later. Two briefs (`DM-GATE-1-diagnosis.md`, `DM-GATE-1a-form-prompt.md`) kept untracked per Tim's call.
- **✅ DM-GATE-1c LANDED v0.33.6 b139 (Tim-authorized in-chat brief, 2026-07-08; Homebase main-checkout).** RL t9 closed: damage/attack context now counts as a "specific target" under H-40's own rule — the META_MODIFIER_FORMULA last resort answers off `meleeProfile` (real dmgMod + atkBonus, weapon named) instead of reciting the breakpoint chart; `answerWeaponDamage` also names the modifier's number ("(+2 for you)"). H-40 preserved (explicit chart demand still serves the table — U653 diverge; U172-23 unchanged). Tests U652 red→green + U653; suite 10946/0; check GREEN; LIVE receipt (Bryn, aldermere, t9 verbatim): _"With the Worn Blade, damage is the weapon's die plus your ability modifier — +2 for you. Your attack roll adds +4."_ — no chart, and it corrects the player's wrong "same 0" claim. Remaining from DM-GATE-1: Cluster B follow-ups (mirror flavor-item · "walk to chest" movement grab · lantern continuity-reconcile · "who wrote this" answerability · villager NPC-combat-eligibility) + **DM-GATE-1b** (persistent object AC/HP, own WORLD_VERSION), all still queued.

**⏳ LANES IN FLIGHT 2026-07-06 ~18:0x (Basecamp):** ALL worker lanes HOME — PW-6 ✅ b123 · SP-3 ✅ b124 · UNIT-CLASH-1 ✅ b125 (Tim's live colossus report, Homebase direct) · ENSURE-STATS-1 ✅ b126 (U628–U629 committed). **MAP-BLEND-1 ✅ b127** (`88bde7a2`, Homebase direct; U630 5/5 — true crossfade, band ×1.12, idle settle-snap; live receipts on Bryn's save). **MAP-VIEW-1 ✅ b128** (`9e1ac9f5`, superseded same evening). **MAP-VIEW-2 ✅ b129** (`59156480`, Homebase direct; U631 relocked 6/6 — curve INVERTED: diorama opens at 0.66 ≈ 50% further, eases to TRUE 1:1 at full zoom (deep end uncapped, spw 6.55299 receipt); + THE ONE-WAY-ZOOM BUG fixed — the orbit overlay had no wheel handler, wheel now FORWARDED to the 2D canvas, proven on the real hit-tested path z=60→2). **MAP-BLEND-2 ✅ b130** (`309f55bc`; the settle FOLLOWS the gesture — U630f; live receipt parked 33.57 IN-gesture → settled 35.84, the b127 nearer-edge rule undid zoom-ins, worst on trackpads — half of Tim's glitchy-zoom report).

### MAP-EGRESS-1 — egress doorstep anchors at the NODE CENTRE, not the drawn building (Tim's cross-town teleport)  ·  map-real pillar  ·  **✅ LANDED v0.32.11 b131 (Tim-gated `ok` 2026-07-07; worker `c56dfe21`+`c4b8f20f` → integrated `a0d28de9`+`25171556`; U632–U635 15/15; suite 10902/0). Layout extracted to engine/world/settlementLayout.js (byte-identical, 4 seeds/66 buildings proven); doorThresholdCells + probe anchor on the DRAWN footprint (the missing falsifier ARMED — proven to catch a reverted centre-anchor); "go out" rider fixed; NO WORLD_VERSION bump; live receipt: Bryn lands ~5 m outside his own Bedchamber/Hearth-Room cottage. Original diagnosis below.** — Proof: boot-home "go outside" writes region (9,4) = wu(45,20) = 14 m east of a storehouse; the drawn home sits at wu(−12,−32) — a 77 m teleport matching Tim's sighting verbatim. Root: `doorThresholdCells` (engine/map/spatial/tacticalPos.js) maps plan layout coords at nodeCentre+L×PLACE_WU while the ink places buildings via the P-81b organic layout (public/map/placeFromNode.js:334–463, rng `${seed}|placelayout`). The standing position probe validates the SAME phantom frame (structFootprintRegionCells) — self-consistent, screen-blind. Fix shape: extract the layout derivation to engine/world/settlementLayout.js (byte-stable rng), renderer re-imports it, doorThresholdCells consumes the true anchor, probe gains the drawn-footprint adjacency cross-check; + one-line: inferInteriorAction misses bare "go out" (rolls an atmosphere d20). U632+ when cut.

### MAP-WEDGE-1 — in-browser QUICKSTART hard-wedge  ·  map/boot  ·  **✅ LANDED v0.32.12 b132 (`309f55bc`-lane; U636 3/3; suite 10905/0). NOT world-gen and NOT random — ROOT CAUSE = `window.confirm` in confirmNewOverSave (public/v1.js) blocking the main thread; DETERMINISTIC on save-presence (first fresh boot skips the confirm = clean; every boot after hits it = block). Proven both directions (save cleared → instant; save present → 30 s timeout on the identical click). FIX = non-blocking in-app modal (ui.confirm + renderConfirmModal); renderer-only. Debugging trap logged: preview_console_logs multiplies logs ~8× per stacked execution context (a capture artifact, not re-execution). FLAGGED out-of-scope: dev-only Export/Import still uses window.prompt (v1.js:2497/2501), same class, later dev-UI pass.** Original queue note: was suspected as a synchronous spin in slice-gen + first map mount — that hypothesis was WRONG; the instrumentation put the hang before beginAdventure ever ran.

ZERO lanes in flight; next free U632+ (MAP-EGRESS-1 claims from there when Tim gates it through).
...
### SOCIAL-PHYSICS (SP-2..6) — the third body of law OPENS  ·  Rung 4 seam  ·  **SP-2 ✅ LANDED v0.32.0 b120 (worker `22074796` → integrated `eb597041`; U614–U616 30/30 w/ canary + migration; suite 10817/0; playtest:quick 50/0 at integration + the worker's 550-run full; WORLD_VERSION 31→32 with the FULL ritual — 16 version-string sweeps, old-save warn+upgrade exercised, ethos derived deterministically from goal keywords for old saves (all 10 pack factions classify right, the merchant league lands neutral), U454-E re-pinned `7fc17002…` and it HELD on the integrated tree (PW-4's S1 invariance kept the base hash still); live receipt: one public torture → civic −10 AND shadow +5 same turn, zero numbers — "the thieves' den warms when the watch curses your name" is REAL).** ENSURE-STATS-1 now UNBLOCKED (the state lane freed). **SP-3 ✅ LANDED v0.32.4 b124 (worker `2c8ebfe0` → integrated `6b596c74`; U625–U627 17/17; suite 10866/0; starting trust reads standing at mint — pure table `engine/social/startingTrust.js`, `5+floor(rep/25)` clamp 3..7; unaffiliated NPCs read notoriety bands (negative-valence only — crimes travel, virtues don't; ceiling 5, floor 3); worker's catch: decompress re-derives trust against the FINAL post-depth factionId, else an NPC could open cold for a faction it left; clean slate mints 5 byte-identical, NO WORLD_VERSION bump; done-when proven live: −60 civic → trust 3, +50 → trust 7 through real aldermere decompression).** SP-4..6 behind per the contract.

### ENSURE-STATS-1 — mintEnemyFromNpc's stats/level are silently stripped (the whitelist trap, live)  ·  combat maintenance  ·  **✅ LANDED same-day 2026-07-06, commit `f661d48a` (U628–U629) — this row was just STALE BOOKKEEPING, never flipped after landing; no new engine work happened 2026-07-10, only the doc correction.** Root: `ensureCombat`'s enemy-rebuild literal (engine/state.js) omitted `stats`/`level` from its fixed field list — `mintEnemyFromNpc` emitted both correctly at mint time, but they were silently dropped the instant the enemy passed through `beginCombat`'s `applyDeltas`, and again on every between-turn `ensureWorld` and every save/load. Verified affected: combat math (`savingThrows.js` — enemy saves fell back to stat-10/level-1 defaults) and moral/death resolution (`deathFact.js`'s `canCommunicate` WITS≤3 gate silently never fired for NPC-minted foes). Verified NOT affected: loot, XP (player-level only), display (nothing reads enemy stats/level today). Fix = smallest possible seam: `stats` via `normalizeEnemyStats` (faithful coercion, NO default-injection — absence is load-bearing for the WITS gate) + `level` clamp 1..20, added to the normalization block + the enemy literal + invariants.js shape-asserts. **RE-VERIFIED 2026-07-10 (diagnosis-first, Tim-authorized):** U628/U629 green 9/9; independent field-by-field parity sweep of every `mintEnemyFromNpc` field against the `ensureCombat` literal — 21/21 present, zero missing (the commit's "every other field already preserved" claim independently confirmed, not just trusted); full ladder GREEN today — 135/135 convergence, 11,046/0 suite (determinism U19/21/22/27/30 included), screen truth locked. **WORLD_VERSION: no bump, correctly** — combat-scoped (enemy objects only exist during an active fight; `defaultCombat()` seeds `enemies:[]`), so the field never enters the boot `worldHash`; the heavily-pinned U454-E boot anchor is untouched, confirmed still green today.
...
### MR-2 — FUNCTIONAL INK (stage 2 design, sliced a–d)  ·  Phase 1/2  ·  **✅ ALL FOUR SLICES LANDED — this header row was just STALE BOOKKEEPING (design doc `docs/briefs/MR-2-FUNCTIONAL-INK.md`, 2026-07-05-eve), never flipped after the children shipped; no new engine work happened 2026-07-10, only this doc correction (mirrors the ENSURE-STATS-1 doc-lag same day).** MR-2a walls-block + door canon ✅ b087 (U503–U506, 28/28; WORLD_VERSION 30→31) · MR-2b DM grounds architecture claims ✅ b088 (U507–U509, 22/22; CG-ARCH detector) · MR-2c house-builder loader ✅ b089 (U510–U512, 24/24) · MR-2d windows are apertures ✅ b093 (U523–U525, 17/17). **RE-VERIFIED 2026-07-10 against v0.35.2/b146 (Tim-authorized doc-only check, not just trusted from the child rows):** `WORLD_VERSION` = 32 (2a's bump intact, later carried by SP-2's 31→32 — consistent history); `DOOR_STATES` enforced in effectsCore.js; `GEOMETRY_BREACH` armed in tacticalPos.js; `CG-ARCH` live in engine/coherence/checks.js (its own comment cites "MR-2b: docs/briefs/MR-2-FUNCTIONAL-INK.md §2b" by name); 2c's loader is the direct ancestor of authoredStructure.js — FUNC-MINIS-1 (this session) built ON that lineage, not around it; roomOccupancy.js's window-facing LOS matches 2d's spec exactly. **WIN-LOOK-1 (queued 2026-07-09) is ADJACENT ROUTING WORK, NOT a reopening of MR-2d:** 2d built the engine's line-of-sight DATA (who's visible through a window); WIN-LOOK-1 is that an explicit player QUESTION ("what do I see through the window?") gets swallowed by the room-survey router before it ever reaches that already-working LOS data — a conversational-routing gap sitting on top of correctly-shipped plumbing.
...
## 2026-07-17 — RULING-DC-1 — the DM judges difficulty (improvised-ruling arc, Fable lane)

### RULING-DC-1 — the ruling rail + the difficulty judge  ·  **✅ LANDED v0.49.0 b170 (Basecamp/Fable, 2026-07-17; engine, Tim-approved Form-Prompt brief; NO WORLD_VERSION bump; executes IMPROV-RULING-STUDY-1 Moves 1+2, unblocks ROUTER-STUDY-1 packet 2).** Two seams. **(A) The ruling rail:** every typed turn persists a compact versioned projection of its grounded intent packet (`data.resolvedIntent` = `{v, source, verb, kind, approach, stat, stake, confidence, target, difficultyBand, difficultyStat}`) on the turn-initiating resolution/blocked event — ONE central write site (`attachResolvedIntent` in the `playerMove` wrapper; all 14+ mint sites inherit), first-annotation-wins, fixed-point projection (`persistableIntent`, U720-B1). Replay (U19/U21 drivers upgraded) feeds a recorded `source:'llm'` packet back through the existing `{llmPacket}` seam — **a model-translated turn replays hash-identical with ZERO model calls** (U720-B2: a deliberately divergent band changes live canon; replay-from-record reproduces it; raw-text replay provably cannot). Old saves (no records) replay by text unchanged (U720-D1); records survive export/import (U722-C1). **(B) The judge:** `{difficulty, difficultyStat}` ride the EXISTING Haiku intent call (+0 API calls, +0 latency; terse per INT-2R), grounded to enums (`DIFFICULTY_BANDS` = trivial|easy|medium|hard|very_hard|impossible; junk coerces null, U722-A2), band → DC through ONE fixed table in resolve.js (5/10/12/15/18; **the model never sets a number** — V11), consumed at exactly two seats on the generic floor: the band prices the feat instead of the global pressure formula, and 'impossible' declines in fiction pre-die (DM Test; no canon write, DENIED 23). Stat precedence: player-declared (DECL-STAT-1) > judge > approach inference (U721-C1/C2). Authored truth outranks: combat/physics/door/social/trivial handlers never consult the band (U721-E1; live receipt: "push the door and try to break the barricade" was claimed by the door machinery at its own DC 12 — the judge's 'hard' opinion recorded in canon with zero authority — while "bend the iron bars" hit the floor at judged DC 18). Files: `engine/intent/intentSchema.js` (+`persistableIntent`/`DIFFICULTY_BANDS`/packet fields), `engine/intent/llmIntent.js` (schema+few-shot+one prompt line), `engine/intent/groundPacket.js`, `engine/playloop.js` (wrapper annotation + hints thread + 2 floor seats), `engine/resolve.js` (BAND_DC + normalize + computeDC short-circuit + stat chain), tests U720 (7) / U721 (7) / U722 (5) red-first, `scripts/ruling-shadow.mjs` (NEW shadow instrument). Proof: suite **11,492/0** · `npm run check` GREEN (135/135, screen goldens locked, boot hashes unmoved) · `playtest:quick` 0 crashes · determinism five ×2 fresh processes · **shadow run (real Haiku): 8/9 fixtures in expected band, 9/9 direction-correct, exploit-direction paraphrase 2/2 stable** (residuals: stuck-cellar-door judged hard — calibration nit; "delicately smash" flipped easy→hard — the HARDER direction, not the exploit) · **live v1 receipt (seed aldermere, Bryn Holt, ear on):** same character, same session — wicker stool `band:trivial → DC 5`, iron well-grate bars `band:very_hard → DC 18` (rolled NAT20, bent them), barricade → door machinery's own DC 12, NAT1, *"Not enough — the wardrobe takes it and holds."* Console clean. Honest boundaries: compound-split inner actions fall back to the formula (hints deliberately don't recurse); the judge rides only LLM-translated turns (offline = today's formula, silent-fallback law); band variance between runs is real (medium vs hard on the same sentence) — bounded by the coarse table, recorded per-turn in canon. Next (queued, needs separate Tim OK): **RULING-FX-1** (Move 3 — light the dark `evaluatePhysics` sealed-vocab consequence path); **router migration** (Move 4) now has its replay rail. Rollback: revert the commit; records are additive event data, no migration either way.

### RULING-FX-1 — the DM narrates the wreckage (sealed-vocab consequence proposals live)  ·  **✅ LANDED v0.50.0 b171 (Basecamp/Fable, 2026-07-17; engine, Tim-approved Form-Prompt brief; NO WORLD_VERSION bump; executes IMPROV-RULING-STUDY-1 Move 3 — the improvised-ruling arc's Moves 1-3 are now ALL live).** The built-but-dark `evaluatePhysics` LLM leg has its first live callers. **Seams:** (1) `groundFxProposal` (llmPhysics) — the ONE consumption gate: object-vocabulary ops only (`modifyFurniture|createItem|removeItem|removeFurniture` — an env/position op poisons the batch; engine owns pressure + position), full `validateDeltas` against the LIVE world all-or-nothing (Invariant 22), census-taker description with `BANNED_RE` drama words rejected; re-runs on EVERY consumption so live pre-flight, replayed record, and forged save get identical scrutiny. (2) The offline physics lane consumes a grounded proposal as CONTENT ONLY — it replaces the material-template deltas + sentence; the seeded `rollPhysicsCheck`, mixed-strips-createItem / failure-voids-all gating, noise, canon `dm.ruling`, and `applyDeltas` all stand (the ruling is the proposal; the dice decide what it bought). (3) The rail: consumed proposals persist as `data.rulingFx` on the physics resolution event; U19/U21 drivers feed the record back through the new `{fxProposal}` playerMove option — replay hash-identical, zero model calls. (4) Pre-flights: `/api/physics-fx` (server, world-blind shape-parse via new `parseFxReply`; Haiku via `intentModel()`, 4s race-cap, 20k payload cap, `PHYSICS_FX=off` escape hatch) + `buildPhysicsFxPayload` (extracted from callPhysicsLLM, browser-safe, doubles as the client's don't-even-fetch gate) + v1.js `tryPhysicsFx` fired in `Promise.all` WITH the intent pre-flight (no latency stacking) + the server `/api/move` path gets the same seat (the paid-gate surface now exercises fx). Files: `engine/llmPhysics.js`, `engine/playloop.js`, `server.js`, `public/v1.js`, U19/U21 drivers, tests U723 (4) / U724 (4) red-first. Proof: suite **11,500/0** · check GREEN (135/135, goldens locked) · `playtest:quick` 0 crashes · offline byte-identical (U723-C1 hash-equality on rejection) · **live receipts (seed aldermere, Bryn, ear on):** "I kick the bed apart" → `/api/physics-fx` 200 → proposal consumed + recorded in canon (`rulingFx`: modifyFurniture + 2×createItem, *"The bed frame splinters under repeated kicks… the straw tick tears, spilling straw across the floor"*) → **roll 5 vs DC 12 FAILED → every proposed delta voided**, template failure line shown, record persisted — the whole law in one turn; retries rolled 9 and a NAT1 (*"You throw your weight against the bed, again and again, but it holds fast"*) — the success path is machine-proven (U723-A1 applies real proposal deltas; U724-A2 mixed keeps state, strips item). Console clean. Honest residuals: one live pre-flight returned proposal:null (cold-start race loss — silent template fallback worked as designed; same payload succeeded on retry); "smash X to pieces" belongs to salvage and "stomp/wrench" phrasings to the generic floor (fx only rides the offline physics lane's verb families — pre-existing routing, correctly out of scope); proposal content varies run-to-run (bounded by the sealed vocabulary, recorded per-turn in canon). Next: **router migration** (Move 4, after OBJ-THROW/BARRICADE) — both rails it needs now exist. Rollback: revert the commit; `rulingFx` is additive event data, no migration either way.

---

## DEATH-TRUTH-1 — destruction leaves an honest, persistent aftermath (Tim's commission 2026-07-16)

*Tim 2026-07-16 (in-chat, engine-brief pre-approved): when something is destroyed or dies it must become a persistent aftermath — real world state, visible and referable, agreeing across mechanics, narration, 2D ink, and the existing 3D path — never a silent disappearance. Three fronts, serial, one lane (Basecamp/Fable main checkout). Visibility bar: the aftermath appears on every surface that drew the living/intact thing. Evidence phase 2026-07-16 reproduced all three terminal paths through production routes; quotable failure: kill a Vein Crawler, ask "is there a body here?" → "No — no body here." — the engine denies the corpse it just created.*

### OBJ-RUBBLE-1 — destroyed authored furniture leaves RUBBLE on the ink (packet 7 of the object-physics arc; absorbs the INK-WRECK-1 upgrade)  ·  **✅ LANDED v0.46.0 b165 (Basecamp/Fable, 2026-07-16; U704 6/6 red-first; U703 evolved to the re-type contract; suite 11,429/0; check GREEN; fresh-process worldHash ×2 equal; live receipt: framed hut room draws bed intact + debris scatter where the barrel stood).** Upgrade the landed suppression to one generic rubble treatment: a destroyed authored piece emits a flagged rubble entry (at its plan position — the immutable authored plan always carries it; evidence: the smash path removes the twin outright, world.objects overlay stays empty, so the plan is the ONE surviving position truth) in BOTH live projections, drawn as a debris glyph, never an intact glyph, never nothing. Cover/blocking untouched (already subtract). PLUS the smallest honest procgen slice — DESIGN CORRECTED IN FLIGHT: the overlay-tombstone idea died on the OBJ-STATE-1 invariant (world.objects records for removed pieces are forbidden by law — invariants.js "names an unknown furniture objectId"); instead the salvage lane's OWN timeline event now carries the piece's stable objectId, and `destroyedObjectIdsAtNode` (engine/objects/query.js) derives the node's destruction memory from that canon + live terminal twins (same ruling as CORPSE-TRUTH-1: aftermath = derived projection over timeline canon; zero new world shape). Procgen rubble/cover/blocking on the map stays BLOCKED on FURN-PARITY-1 proper (see corrected row) — drawing procgen rubble without the Model A↔B join would be a renderer guess.

### FURN-PARITY-1 — EVIDENCE CORRECTION (2026-07-16, supersedes the "identity half absorbed" optimism): Model A procgen furniture (decompression/generateFurniture.js generateNodeFurniture) is generated from its own TEMPLATES list on its own RNG stream — fully INDEPENDENT of the Model B roomDetail role-loadout the map/cover/blocking derive. The pg:<nodeId>:<slot> id names a Model A piece with NO Model B counterpart; roomObjects.js assigns by seeded AFFINITY guess. Full parity = re-architecting the procgen generation seam to enumerate roomDetail loadouts (mirroring seedAuthoredNodeFurniture) — changes which pieces exist in procgen worlds (worldHash/byte-identity/save implications). Still QUEUED, now with the destruction-memory foundation from OBJ-RUBBLE-1 (the overlay record is the identity-based join's future source). tacticalPos.js:382's own comment already states the boundary: live blocking is "authored only; generic/procgen movement waits on Model A↔B identity parity."

### CORPSE-TRUTH-1 — the dead leave locatable remains (monster + NPC)  ·  **✅ LANDED v0.46.0 b165 (Basecamp/Fable, 2026-07-16; U705 7/7 + U706 4/4 red-first; suite 11,429/0; check GREEN; live receipts: “Is there a body here?” → “Yes — the dead lie here: the Vein Crawler, Jorin.” on the exact question that used to deny the corpse; dead Jorin draws the fallen mark on the walk map, in-page projection dead:true; roster row → ‘Jorin — dead’).** FRESH-EYE FIX v0.46.1 b166 (same day): the cold review caught the one-authority/many-surfaces miss — oneMap drew rubble as an intact-looking box and dead NPCs kept standing on oneMap/the 3D diorama/interior sheets; all four surfaces now consume the same flag (rubble debris branch + fallen marks; 3D lays the figure down, faded, no breathe), U576a byte-locks evolved with dated carve-outs, U706-C2 pins the 3D people feed. Representation ruling: remains are a DERIVED PROJECTION over death-fact canon (timeline 'death-fact' events — already the honest lazy-additive store, deathFact.js's own words), unified with the existing NPC corpse truth (meta.npcCombatHp down + roster). NO new ensureWorld field, NO WORLD_VERSION bump. Additive mint fields: fact.nodeId (where) + fact.victim.archetype (what kind — feeds the existing per-creature corpse-mini seam later). New canonical helper remainsAtNode(world, nodeId) (dedupes fact-derived + roster-derived by victim identity). Consumers: presence/seek reads for corpse nouns (the "is there a body here?" denial seam), the look survey (a remains line, SEPARATE from the furniture/objects list — corpses never join take/salvage surfaces), sense-death (already reads roster NPCs; monsters join via facts). 2D LocalMap: a dead NPC stops drawing as a living figure (public/map never consulted death state — evidence: zero npcCombatHp reads) and draws a corpse mark. Boundaries (reported, not hidden): pre-feature death facts lack nodeId ⇒ old kills stay unlocatable; living monsters never drew on the 2D map ⇒ monster corpses don't either (prose + presence own that truth); the 3D board's existing corpse minis (figures3d corpseTemplates, deterministic per-creature + toppled fallback) stay combat-scoped — board re-entry persistence is future work.

### ⛔ DEATH-TRUTH-1 FINISH — **REOPENED 2026-07-16 (correction in flight; the b167 "closed" claim below is WITHDRAWN).**

#### DEATH-TRUTH-1c (2026-07-16, worker `death-truth-1c`, v0.47.1 b168) — **defect 3 CLOSED (canon). Tim's §0 bar NOT met — the picture is unfixed.**

**CLOSED — the canon side of defect 3.** b167's ambush anchor (`playerPos + anchorRng.int(-3,3)`) was measured over 120 seeds on the production route: **0/120** agreement with the board, **80/120 (67%)** on the wrong side of the player, median **6 cells (30 m)** out, and the **killer's own square** on seed `scan48` — the substitution b167's own law forbade. The scatter is DELETED, not repaired. Replaced by a real board **origin** (`grid.js boardOriginFrom`): the player's board cell is pinned to the player's canonical pos, so every cell has a true world address by construction, 1:1 (UNIT-CLASH-1 — the "5-ft square" is rules-flavour; no ft→m conversion near a cell). The fact reads the projection at assembly time ⇒ the DEATH cell by construction. **After: 120/120, 0, 0.** U711 (5) + U713 (6), value-assertions with anti-vacuity guards; U711-B/C proven to FAIL against the re-introduced defect. Suite 11467/0 (+11). Boot worldHash unmoved. Two whitelists carried (`ensureCombat` **and** the `effectsCore` combatState merge — the brief only knew of the first).

**NOT CLOSED — the picture (brief Phase 3). Built, measured, REVERTED.** Tim's bar is "the corpse is right where it died… at every zoom", and it is **not met**. Root cause is bigger than "the corpse path scatters": **canonical positions are decorrelated from the drawn sheet for EVERY entity.** Measured at Trader's Camp (seed loaderDemo): buildings span uy −2.4…21.7, but **every** NPC's canonical pos projects to uy 52…70 — not one lands inside the settlement it lives in; the player's own canon (uy 23.5) ≠ where the player's token draws (uy 12.2). Living NPCs draw from a road scatter that never reads `npc.pos`. Routing only the corpse through canon therefore drew the body **~230 m outside the village** — a visible regression, so it was reverted. `npc.pos` is not a position in the drawn world; the geometry was never the blocker (the `placeUnitToRegionCell` inverse is ~5 lines and was written and proved, then removed as dead code). This is the repo's known **"map marker reads v1 walk-pos, not engine state"** seam and needs the canon↔sheet reconciliation — a renderer+v1 packet, explicitly out of scope here ("No renderer rewrite").

**ALSO WORSE THAN DESCRIBED — the settlement-only gate.** Not one gate but the whole surface: on seed loaderDemo **24 of 25 nodes carry no settlement object** (5 wilderness, 10 landmark, 4 dungeon_entrance, and 5 nodes *typed* `settlement` with no settlement object). `drawModel.placedTokenModel` gates on `node.settlement ?` and returns `people: []`, so **no corpse draws anywhere but the single real settlement** — Tim's wilderness bar fails on 24/25 of the world.

**QUEUED (needs Tim / a renderer lane), in dependency order:**
- **DEATH-TRUTH-1d — canon↔sheet reconciliation (the real blocker).** Decide the ONE truth: either the sheet draws living entities from `npc.pos`/`party.pos` (canon authoritative, POSITION_AS_CANON honoured, the scatter retires), or `npc.pos` is admitted to be non-spatial and the drawn scatter is promoted to canon. **This is a canon-semantics ruling — Tim's call, not a worker's.** Until it lands, NO corpse-position work can meet the §0 bar, and b167's "the fact carries the true pos in canon" is true but worthless to the player.
- **DEATH-TRUTH-1e — corpses outside settlements.** Needs a frame-free placement path (`regionCellToWu` is node-anchored and needs no settlement frame) + removal of the `node.settlement` gate (drawModel.js:657) and the `nodeType !== 'settlement'` gate (render3d.js:841). Blocked on 1d (no point drawing a corpse truthfully on a sheet whose other entities lie).

**⚖️ DEATH-TRUTH-1d RULED 2026-07-16 (Tim, in-chat: "proceed") — dispatched same day (worker lane `death-truth-1d`, brief `docs/briefs/DEATH-TRUTH-1d-people-become-places.md`; 1e FOLDED IN).** Basecamp's diagnosis: the 1d binary was a false choice — the repo has ruled this three times already (DEC-1 footprints → MAP-EGRESS-1 buildings → FURN-PARITY-1 furniture), always the same way: *the drawn geometry is promoted to engine truth and both surfaces read it.* Root mechanism found: `placeNearNode` (`tacticalPos.js:1143`, `NODE_JITTER=50` = **±250 m** box around node centre, blind to the settlement layout) is TAC-1 scaffolding that was never upgraded — it alone explains every measured number in the 1c row above. Everything needed already exists: `placeUnitToRegionCell` (the MAP-EGRESS-1 lattice, `settlementLayout.js:343`) IS the shared coordinate lattice `placeFromNode.js:245`'s comment claims doesn't exist (comment is stale), and `playerTokenPlaceUnit` (MR-1b) ALREADY reads canon, falling back only on region-frame pos because region canon was jitter-garbage. **Keystone:** 1c's board origin hangs off the PLAYER's canon, which outdoors is the same jitter — grounding the player's outdoor canon is the first move; every death fact becomes sheet-true automatically. Ruling executed = people-scatter promoted to engine (byte-stable, OCC-STORY-1 worldHash-re-pin protocol), jitter deleted, renderer reads canon, settlement-only corpse gates killed. The four 1c §5 screenshots are the acceptance bar.

**✅ DEATH-TRUTH-1d + 1e LANDED v0.48.0 b169 (worker `death-truth-1d`, 2026-07-16; NOT pushed — Basecamp lands it).** The ruling above, executed. The people-scatter is now engine truth (`engine/world/settlementScatter.js`, byte-identical to the old draw — proven 20 seeds × 2 frames, U714); `placementForWorld` grounds every outdoor settlement occupant at the exact place-unit the sheet scatters them to → `placeUnitToRegionCell`; the player's outdoor settlement canon = the drawn lane entry (keystone — the board origin pins to it, U717); wilderness entities take a bounded walkable near-node scatter; `placeNearNode`/`NODE_JITTER` deleted. The renderer READS canon: new `regionCellToPlaceUnit` inverse, `playerTokenPlaceUnit` region branch flipped to projection, living NPCs + corpses drawn from their region cell (a villager and the body they become share ONE coordinate — U716). 1e fold: wilderness corpses draw frame-free via `regionCellToWu` on the 3D diorama AND the 2D oneMap sheet; the `node.settlement`/`nodeType!=='settlement'` corpse gates are killed (U718). Interior corpses draw at the struct-frame cell the fact froze. **Boot worldHash moved ONE documented step (settlement-true NPC values): tallow `2fdf7b82…→1483b7da…` (U454-E), loaderDemo `8e5992e4…→a2bb740b…` (U713-E); NO WORLD_VERSION bump, no migration** (pos SHAPE unchanged; old saves keep their pos, new worlds are true). Replay determinism green (U19/U21/U22/U27/U30), two fresh OS processes agree; the ≤2.5 m integer-cell snap is the accepted §3f cost (U494 proves no token in a wall; U719 bounds the round-trip). Tests U714–U719 (value-assertions, anti-vacuity guards); suite 11473; 5 screen goldens re-blessed (0.78–5.27% position shift, no degradation); U576a/U576-oneMap carve-outs extended (established per-packet pattern). **§0 bar — the picture half:** MET on data + a live worktree-build receipt (seeded outdoor-corpse save: in-browser the Dire Wolf corpse draws at (12.15, 21.01) from its canon cell (−801,−392); npc_0's corpse at (9.65, 6.01) where it stood; player at canon (5.9, 23.51); the settlement renders coherently). Honest residuals (do NOT reopen without reading): interior LIVING people still draw at a room-scatter angle (only the interior CORPSE reads the true cell — pre-existing, the corpse-where-it-died bar still holds); the crisp fallen-mark pixel at max zoom wasn't isolatable live (player-centered camera, no pan) though the in-browser token data is definitive; the §7 open question (5 m square vs a 1.83 m mini) is Tim's scale ruling, untouched. **The six OTHER reopened DEATH-TRUTH-1 defects stay REOPENED** (furniture split-brain #1 is its own WORLD_VERSION-bump packet; save/load recovery; creature corpse art). The b167 withdrawal wording above stays.

**OPEN QUESTION FOR TIM (flagged, not decided — brief §7):** the repo has ruled a tactical square is **5 m** and "5-foot square" is rules-flavour. Consequence: a foe 3 squares away is **15 m ≈ 49 ft**, and a 1.83 m human mini stands in a 5 m square (~2.7× wider than they are tall). Not surfaced with a max-zoom screenshot this packet — the corpse never reached a legible zoom, because of the 1d blocker above.

**Combat movement — a correction to the brief's premise.** The brief's primary hypothesis ("a creature charges you, so the corpse records the treeline") is **false for the live engine**: enemies never move. `moveCombatant` (grid.js) is called for `'player'` only (escapeCombat.js:999); `spatialMove.js` resolves the PLAYER's token. Spawn cell == death cell for every foe today. The fix does not depend on that (it projects at assembly time), but the brief's "spawn ≠ death" framing described a bug that does not exist — the real one was that the recorded position was decorrelated from the board entirely.


**The b167 rows in this section claimed completion that independent review disproved. Treat every "✅ LANDED v0.47.0 b167" claim below as UNVERIFIED until re-earned.** Seven defects were reproduced against the production routes at `24d6433d`; all seven are confirmed, several worse than first reported. This row is the honest status; do not restore completion wording until each is proved.

1. **Furniture parity is still split.** `seedProcgenNodeFurniture` appends `au:` plan twins BESIDE the legacy `pg:` pieces (`decompress.js:59`/`:192` write pg: only when `node.furniture` is empty; at boot pg: wins and au: is appended after). pg: pieces have no drawn twin — every render/cover/blocking surface keys off the plan's `pieceId`, which pg: lacks. Outdoors, `roomObjects.js:174` (`if (!interior) return finish(legacy)`) hands back the WHOLE node furniture list, so a player outside can smash a wardrobe through a wall — and `tests/U307.roomScopedObjects.test.js:150` currently ASSERTS that leak as correct ("outdoors sees the full node list"). ROM-4 name-uniquing (`authoredFurniture.js:274`/`:376`) mints `shelves 2`, whose head noun under `llmPhysics.js:87-90` becomes `"2"` (`.pop()`), failing the `length >= 3` gate — so `shelves 2` is permanently unaddressable by name.
2. **v34 and already-created v35 saves are not repaired.** `save.js:17-41` warns and returns `ensureWorld(parsed)`; nothing seeds. `ensureWorld` never calls `seedProcgenNodeFurniture`. `procgenPlanPieceStatus` returns `seeded:false` → "treat every item as intact", so a resumed save draws all 16 plan pieces while exposing ZERO plan-sourced objects. `playloop.js:4397` is gated on the node CHANGING, so a resumed player standing still never recovers. The gear-menu Load button (`v1.js:2422`) shares the gap. Nothing persists post-load, so the repair must key off the per-node `furnitureSeeded` marker, not `meta.version`.
3. **Death facts recorded the player, not the victim** — ✅ **CORRECTED (see below).**
4. **Renderers ignore canonical corpse position.** FOUR independent position authorities, three of them separate hash-scatter implementations with different salts: `placeFromNode.js:403-406`, `LocalMap.js:691-693`, `LocalMap.js:967-970` (+ the board's `combatScene.js:67-74`). None reads `loc.pos`. Note `loc.structureId`/`loc.roomId` ARE honored — it is specifically sub-room precision that is dropped. A canonical projection already exists and goes unused: `worldSpace.js:293 regionCellToWu` / `resolveEntityWu` ("ONE address for any entity").
5. **Monster corpse identity is incomplete.** `interiorTokens.js:42` drops `archetype`/`corpseKey`. `miniLibrary.js:28-37` is the WHOLE corpse pool — two generic gore assets — and `pickCorpseMini` (`figures3d.js:497`) uses the key only as 2-way hash entropy, with no archetype parameter. The async path degrades BACKWARDS (`figures3d.js:525` → `render3d.js:857-860`): the fallback shows the species-correct toppled creature, then the resolved GLB REPLACES it with a generic human gore pile. Persistent projection is settlement-only in TWO layers (`placeFromNode.js:320-321`, `render3d.js:841`), so wilderness/dungeon kills are canon but invisible. `corpseKey` is an identity string, not a species key — wiring alone cannot fix this; the asset library needs per-archetype extension.
6. **Invariant #14 is breached.** The b167 row below self-flagged it ("⚠ flagged vs IMMORTAL_INVARIANTS #14 — Tim to bless or revert") and it was never blessed nor reverted. Procgen interiors bypass the authored catalog via `floorPlanToSceneModel`.
7. **Render-path performance regressions.** `destroyedObjectPositionsAtNode` calls `ensureWorld()` during rendering; `procgenPlanPieceStatus` rescans node furniture + the append-only timeline per structure; `objectsHere()` repeats global furniture lookup.

**✅ Defect 3 CORRECTED (this commit) — the death fact records the VICTIM.** Root cause: `deathFact.js` copied `world.party[0].pos` under a comment claiming it was "the melee anchor the engine actually owns". Reproduced on seed `loaderDemo` through the production route: player `(-806,-390)`, Jorin `(-800,-353)`, fact recorded `(-806,-390)` — the killer's position, stored as the victim's. The suite stayed green because `U710-A` only asserted the pos was SHAPED right (integer gx/gy, frame `region`), never that it was the victim's. Fix: new `victimWorldPos(world, victim)` resolves, in order — (1) an explicit victim-owned `worldPos` minted at the encounter/placement seam, (2) an NPC-sourced foe's own canonical roster `pos` (POSITION_AS_CANON — always present, simply unread), (3) honest `null`, degrading to node-level truth exactly like a legacy fact. NEVER the player. The anchor is established AT SPAWN, not manufactured after death: `mintEnemyFromNpc` inherits the NPC's real `npc.pos` (no invention); `beginCombat` gives a bestiary foe that owns no position a deterministic engagement anchor off the same placement seed, before any death. Carried through `ensureCombat`'s enemy whitelist (`state.js` — the same trap that ate `traits`/`stats`), combat-scoped and added only when present, so the boot world stays byte-identical and the boot worldHash does not move (U454-E green). Red-first `U710-A2` asserts the fact equals the victim's pos AND differs from the player's, and fails if the two are not genuinely distinct. **Honest limitation:** a bestiary foe's anchor is its SPAWN position — the abstract board it then manoeuvres on has no world projection, so spawn is the most specific location the engine honestly owns for it. NPC victims involve no invention.

*Superseded b167 claims, retained for the record (the wording below overstated what landed):*

**FURN-PARITY-1 → ✅ LANDED v0.47.0 b167 (completes the corrected row above).** The re-architecture the correction called for, done at the seam it named: a PROCGEN structure now seeds its roomDetail loadout into node.furniture as REAL Model A pieces (`seedProcgenNodeFurniture`, mirroring seedAuthoredNodeFurniture; chained at applyGeneratedStructuresForNode, once-per-structure via the same furnitureSeeded marker) with provenance `{ planSourced, structureId, roomId, pieceId }` and identity `objectId = au:<structureId>:<planPieceId>` — the au: namespace = structure-scoped provenance identity; the `authored` FLAG stays Builder-only, so every supported-domain verb gate (hold/drag/throw) is unchanged. KIND_PHYSICS extended to ALL 41 FURN kinds (U707-A fails closed on a new kind). One status authority `procgenPlanPieceStatus` (destroyed → rubble · taken → nothing, never rubble · unseeded → intact) consumed by cover (coverFeatures), blocking (liveAuthoredBlockedCells), the village sheet (flattenRoomFurniture), and the interior scene (buildAuthoredSceneFurniture, generalized). PROCGEN INTERIORS now render ENGINE TRUTH (floorPlanToSceneModel on the real floor plan + live furniture) — the catalog-art index-mapping branch is retired for topology structures (⚠ flagged vs IMMORTAL_INVARIANTS #14: the pipeline is still the hand-drawn createInteriorMap; what changed is WHICH truth it projects — Tim to bless or revert). WORLD_VERSION 34→35 (world-GENERATION change: which pieces exist in procgen worlds; old saves warn once, visited nodes seed on next arrival, nothing resurrects); U454-E boot-hash re-pinned (anchor chain … OBJ-DURABILITY-1 e1b89d6f… → FURN-PARITY-1 2fdf7b82…). Tests U707 (7/7 red-first). Residual honest boundary: the 2–4 generateNodeFurniture TEMPLATES still seed per settlement node (prose-level objects with no drawn twin — pre-existing; removing them is a separate ruling).

**OBJ-RUBBLE-1 terminal-location CORRECTION → ✅ LANDED v0.47.0 b167 (supersedes the b165 row's "at its plan position — the plan is the ONE surviving position truth"; that was only true for pieces destroyed WHERE THEY STOOD).** Evidence: drag the loaderDemo barrel (58,54)→(57,53), salvage it — rubble drew back at the plan anchor. Root: effectsCore removeFurniture deletes the overlay record (the only holder of a moved position) and the salvage event carried no position. Fix: trySalvage captures `resolvedObjectPlacement` BEFORE the removal deltas and stamps `data.pos` on the salvage event (additive; old events lack it and honestly mean "died at the plan anchor" — correct, nothing could move pre-OBJ-MOVE-1); `destroyedObjectPositionsAtNode` (objects/query.js) is the canonical read; both projections position rubble by live-placed-overlay (standing wreck) > event pos (removed piece) > plan anchor. Renderers PROJECT the evidence, never reconstruct it. Tests U708 (6/6 red-first: moved+salvaged · unmoved pin · moved+wrecked-in-place · thrown-then-destroyed at the landing cell · save/load · replay-hash).

**CORPSE-TRUTH-1 COMPLETION → ✅ LANDED v0.47.0 b167 (closes the b165 row's two reported boundaries: "monster corpses don't draw on maps" and node-scoped-only location).** The fact now captures the killing moment WHOLE: `loc = { structureId, roomId (scene.interior), pos (the player's canonical tactical pos — POSITION_AS_CANON) }` + `victim.archetype` (deriveArchetype moved VERBATIM to engine/combat/creatureArchetype.js; combatScene.js imports it back — one authority) + `victim.corpseKey` (the exact key the combat board hashes for its corpse GLB). The abstract combat grid's cells are deliberately NOT recorded (that board dissolves at endCombat — never fabricate precision). Every surface projects it: located dead LEAVE the living feeds and return as PINNED corpse tokens (identity-keyed seed, no list-order dependence — a roster change can never re-scatter a body); interior sheets draw the body in its DEATH room (new 'dead' token type in handDrawnInterior; drawInteriorV2 emits them, knowledge rule holds); the village sheet pins outdoor deaths (indoor bodies stay off it, like living indoor folk); the 3D people feed carries archetype+corpseKey and render3d shows the creature's OWN corpse (buildCorpseMini GLB → toppled-archetype fallback → laid-down villager; also fixed the b166 latent bug where a lying figure was sized by its post-rotation y-extent ≈ 5–15× too big). Presence answers are LOCATION-SCOPED: "Is there a body here?" answers where you STAND (interior room match / open ground) with an honest pointer otherwise ("Not here — Jorin's body lies outside.") — the caught lie; a new outdoor corpse-noun lane answers in the open. LEGACY dead (npcCombatHp only / pre-feature facts, no loc) keep the old node-level behavior on every surface — degraded honestly, never assigned a guessed room. Tests U709 (5/5) + U710 (6/6) red-first; U706 unchanged and green.
