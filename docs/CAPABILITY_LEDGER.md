...
**Phase C (populate) — IN MOTION (2026-06-24).** Decisions 1 (all-bespoke **Opus 4.8 voice for every NPC**) + 2 (the 5-figure roster: Aurelius/Joan/**Kant×Knight**/Goldblum-Socrates/**Jesus-the-Host** + the Twain-**Goat of the Blasted Heath** wildcard) both RESOLVED; figure voice archives authored in `server/rag/corpus/`. Decision 3 (orb climax) reshaped → the departed-orb + ALIENS-hive + fragments-as-police (`DEMO_REGION.md` §8). **D-C1 — NPC voice wiring: LANDED ✅** (`e6c41fd`, Opus worker → Basecamp §7-verified + live-proven end-to-end + pushed): ordinary NPCs now speak Opus-grounded lines via the `/api/npc-voice` route (engine stays pure/sync — surfaces `voiceCorpusId`; silent fallback Opus→8B→templates; `N8`, suite 8710/0). **D-C2a — authored-figure overlay + steward-king: LANDED ✅** (`1ab5de2`, Opus worker → Basecamp §7-verified + live-voiced + pushed): new `engine/world/demoFigures.js` overlay (tallow-gated, deterministic, idempotent) injects authored figures at decompress; **Theodore Augustus** (Aurelius voice) now stands at the kingdom seat in `tallow` and speaks (`U265`, suite 8713/0). The other 5 figures sit inert in the table (`nodeSelector:null`). Next: **a node-role map** — which procedural `tallow` node is the frontier / faith / trade / cannibal-outlier town + the Goat's wilderness (a taste call, then place the other 5 via the same overlay) + the **bulk 437-archetype→corpus mapping** (D-C1 covers 25 common roles).

**Parallel wiring sprint — Lane "Light Up the World" LANDED ✅ (2026-06-24, `6dc951a`, clean FF, +37 tests).** First of the two worker lanes (`docs/TWO_LANE_PLAN.md`) handed up all three packets in one branch, Basecamp §7-verified (clean FF off `ce9086d`, diff stayed inside the fence, no hot-file/`state.js` crossing, lane-check + `npm run check` green: convergence 109/109, suite **8752/0**, determinism incl.). **W1·1** broadened the voice allowlist past the 25-role seed (`U270` 7/7). **W1·2** deepened `placeQuery`/`personQuery` so pointed substrate questions resolve from buried history or decline in-fiction (`U271` 17/17). **W1·3** filled the cross-lane `rumorsReaching` contract — tiered, traceable gossip + player-deed reputation (severity ≥ 25) synthesized at read-time, pure/deterministic (`U272` 13/13; my `U266` contract guard still green). **The W2·3 dependency is now CLEARED** — the rumor producer is real before the consumer builds. Lane "Make Every Turn Honest" (`honest-turn/p86-combat`) still running; not yet handed up.
