...
test('G11-01: all three McCarthy arcs validate and load (4 arcs total)', () => {
  for (const a of [theJudge, theWages, theFire]) assert.deepEqual(validateArc(a), [], a.arc);
  const ids = getArcs().map(a => a.arc);
  assert.deepEqual(ids, ['the-cold-well', 'what-the-fire-left', 'the-judge-passes', 'the-wages-of-blood']);
});
