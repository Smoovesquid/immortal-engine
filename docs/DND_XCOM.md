...
- **DX-1 · narrate-the-read in the DM prompt — ✅ DONE (`881cb14`).** Added the TACTICAL READ rule to the combat branch of `buildSystemPrompt` in `engine/llmAdapter.js` (surface threat/footing/cover/advantage/options as fiction; never HP/%/DC/labels). Contextual, additive, suite 8941/0. *Remaining:* run the convergence / Opus gate to confirm narration quality (costs API — not auto-run; budget). The `qualityJudge` `agency` criterion already guards the negative half (no leaked numbers) — leave its 6-criterion calibration alone.
- **DX-2 · wire the XCOM combat core into the live path.** **`engine/tactical/combat.js` ALREADY IS the XCom cover/hit core** — `coverAt()` (directional cover/flank), `hitChance` (cover+range+flank), deterministic `resolveAttack`. The **live D&D path** is `engine/combat/combatResolve.js` → `resolveCombatTurn` using `computeAttack/computeAC` (`engine/gear/gearProps.js`) + traitHooks/bossActions. **Work:** decide the bridge — committed positions feed cover→AC bonus / flank→advantage into `computeAC`/`computeAttack` (or route through `tactical/combat.js`), so position confers mechanics, then surface it via DX-1's read. **Determinism-sensitive + competence hot file → Codex lane, with the combat gates.** (MAP_PATH Phase 2.3.)
