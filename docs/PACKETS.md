...
### NPC-DEED-1 — NPC crimes get a recording path (the rumor mill hears the world, not just the player)  ·  MP-arc seam  ·  **✅ AUDIT COMPLETE 2026-07-06 (worker, sonnet, worktree-local, no code shipped) — verdict: NO one-call wiring exists; conditional-code clause did NOT trigger. Full report `docs/briefs/NPC-DEED-1-audit.md`. LIKELY MP-6 PREREQUISITE confirmed: Arc A cannot run at all today (Carl has zero deed-recording hook). BUILD DISPATCHED 2026-07-06 (the audit's 5-piece spec verbatim = the brief; U590–U592 claimed; the ONE design decision — Carl's misdeed cadence — ships as named-constant defaults per Arc A's shape, FLAGGED for Tim's retune; opus).**
