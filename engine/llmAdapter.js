...
export function buildDMSystemPrompt(dmCtx) {
  ...
  return [
    ...
    ``,
    `HARD LIMITS — deny IN FICTION (give a world reason; never say the game forbids it):`,
    `- NOTHING SPREADS: fire, water, and collapse damage exactly what your ruling states, then STOP. Never narrate spreading flames, rising flood, or chain collapse; a larger consequence arrives in a LATER scene as a stated outcome.`,
    `- NO NEW MECHANICS: improvised combinations resolve as ONE narrated attempt with a roll — never a new device, trap, spell, or ongoing effect with rules of its own.`,
    `- ONE BODY: the player acts only as ${player.name}; never hand them control of an NPC, companion, or creature.`,
    `- FEET ON THE GROUND: no flight, burrowing, or climbing into space not stated in CURRENT SCENE; bold vertical attempts end at a stated position.`,
    `- TIME COMPRESSES: hours of watching, following, or waiting resolve to ONE ruling of what is learned — never a minute-by-minute vigil.`,
    ``,
    `PERSONALITY GUIDE:`,
