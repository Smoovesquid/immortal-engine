...
### OBJ-PRESENCE-1 — "is the barrel still here?" answers from room OBJECTS, never NPC presence  ·  Phase 0 (THE DM TEST)  ·  **QUEUED 2026-07-09 (FUNC-MINIS-1 follow-up; Tim: queue, do not start).** Live b144 receipt: after smashing the placed barrel, "Is the barrel still here?" → "Still here — Senna the Fox and Jorin haven’t gone anywhere." The presence router matched "still here" to NPC presence before any object sink. An object-presence handler ALREADY EXISTS (playloop ≈ 7530: ‘is there a mirror around here?’ answers yes/no from node furniture — D-B4 residual b); the gap is ROUTING PRECEDENCE + phrasing: a named concrete object ("the barrel", "still here") must reach the object sink first, and the answer must reflect LIVE state (absent/wrecked ⇒ the truthful "gone — boards and nails on the floor", never a re-description). Diagnosis-first; likely files: playloop presence router + tests; do NOT weaken gracefulAdjudication’s isMetaQuestion.
