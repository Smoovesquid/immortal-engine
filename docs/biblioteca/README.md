...
### Vol 16 — Automated Playtesting & Lived-Coherence Harnesses  →  [`vol-16-automated-playtesting-coherence-harness.md`](vol-16-automated-playtesting-coherence-harness.md)  ·  *2021–26 frontier; the play-the-game-to-find-bugs sibling of Vols 8/9/15*
*How others build agents that PLAY a game to surface bugs, and how to catch cross-turn INCOHERENCE in long-form interactive narrative — the axis the per-turn DM-text gate is blind to ("tests well but doesn't make sense").* **Headline: the Human Playtest Harness is an ASSEMBLE-not-invent pattern.** Built ≥2× in the last year — **TITAN** (4-oracle LLM game-tester: crash / stuck / slow / logic + reflective stuck-detection; beats human state-coverage 73% vs 34%, 82% bug-detect) and **Lap** (thin perceive→prompt→act, 79% vs 3–5% coverage); our **exact class** has a paper (**Jin et al., ACL 2024** — detect hallucination / forgetfulness / misinterpretation from LLM-text-game logs); and the coherence-judge half is **solved** — **ConStory** (5-category / 19-subtype incoherence taxonomy + extract→pair→evidence→JSON checker that **beats human experts F1 0.678 vs 0.229**). **Two reframes:** automated coherence-checking is *better* than playing-by-hand (answers "so I don't have to playtest forever"), and the literature's hard part (perception) is **free for us** (text + Canon Log) — our judge checks prose against the *actual state* (SCORE bolts on what we already are). **Road-A-safe:** judge proposes, Canon Log commits. Maps onto existing assets (`scripts/playtest.js` crash/perf oracles, `dm-playtest.mjs` player/judge scaffold, THE_REF's `validateNarrationCandidate`) + the capability ledger (ConStory subtypes ≈ C8 / C9 / CRUNCH_INCONSISTENCY / CANON_HALLUCINATION). Build shape: two tiers (fast engine-loop + thin browser), a **typed-goal** player (not a vibe-persona — drift caution), a soft-lock/goal oracle (TITAN Task-Status + reachability lineage: Go-Explore / SEED / CTL), a CED-style **asymptotable** meter. **Reach for it:** scoping the Human Playtest Harness, any coherence / soft-lock / goal-completion oracle, or "play end-to-end to find bugs" tooling. Build plan: `docs/HUMAN_PLAYTEST_HARNESS.md`.

### Vol 17 — DM Narration Craft → the Quality Rubric  →  [`vol-17-dm-narration-craft.md`](vol-17-dm-narration-craft.md)  ·  *2026; the voice/table-feel layer behind Vol 16's quality judge*
*How real DMs actually TALK — distilled into named judge criteria + DM-prompt guardrails.* **Headline: "how a good DM narrates" is a documented, named craft (not vibes), and naming it does DOUBLE DUTY — it sharpens the quality judge AND fixes the DM prompt that generates the prose.** Four canons converge: **PbtA MC principles** (Apocalypse World — *address the characters not the players · make your move but **never speak its name** · name everyone make them human · be a fan*), **The Angry GM** (narration = concise info + a call to action; ≤5 sentences; no "filtering" / "you see that…"; speak normally not Tolkien; **never narrate the player character**), **Johnstone's *Impro*** (yes-and / don't block the offer), **Sly Flourish** (strong start; describe through the eyes). Distilled — **scoped to narration only** (the engine owns adjudication; only voice-craft transfers) and filtered through the invariants — into the **6 named rubric criteria** now in `engine/harness/qualityJudge.js` (resolves-the-intent · respects-agency · no-machine-voice · concise-no-filtering · specific-and-grounded · natural-DM-voice) + **2 DM-prompt guardrails** in `buildSystemPrompt` (HIDE THE MATH ← "never speak its name"; RESPECT AGENCY ← "never narrate the player"). **Proven live:** the hide-the-math rule killed the "forty-four souls / the economy hums" stat-dump (A/B), and the named rubric caught an agency/wrong-NPC routing bug the generic 3-criterion version missed. **Caveats:** single-call judging has a halo effect (Autorubric warning) → per-criterion calls = the calibration follow-up; the judge stays a discovery pointer (human-triage, never auto-fixed); taste stays human. **Reach for it:** tuning the quality rubric, editing the DM system prompt, or any narration-voice / table-feel work. Sibling to Vol 15 (LLM-GM authoring) + Vol 16 (the harness that measures it).

*The arc: Vol 2–6 build the layers, Vol 7 wires them, Vol 8 tests them, Vols 9–10 calibrate the testing; **Vols 11–14 = the 2025–26 frontier layer** — new pragmatics (11), fragility/leakage (12), secret-leakage DEFENSES (13 → C16), current judge+coverage SOTA (14); **Vol 15 = the LLM-GM / interactive-drama frontier** (2023–26) — the games-side endorsement of "GM proposes / system commits", plus GM-moves-as-affordances + possibility-space simulation; **Vol 16 = the automated-playtesting / lived-coherence harness frontier** (2021–26) — the play-the-game-to-find-bugs sibling (TITAN / Lap / ConStory / Jin), the measurement axis the per-turn gate misses; **Vol 17 = the DM-narration-craft layer** (2026) — the named voice-craft (PbtA / Angry GM / Johnstone / Sly Flourish) behind Vol 16's quality judge + the DM prompt. The frontier vols are SOTA-dated; re-mine periodically.*
