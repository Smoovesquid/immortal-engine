...
## Budget — **~$4.44 estimated post-gate-4** (was $7.29 Tim-confirmed pre-gate-4) — near the ~$2.5 one-run floor; RE-CONFIRM with Tim before the next gate
...
would drop below ~$2.5). **Gate 4** (2026-06-21, post H-75/76/77, ~$2.85, `docs/playtests/opus-gate-2026-06-21.md`,
**7/48, 0 new capabilities — 3rd consecutive zero-discovery gate; C12 + C7 fixes held live, the bounce is the deferred
C4 tail**) → **~$4.44 estimated left.** At ~$2.7/run that's roughly ONE gate of headroom — re-confirm the real balance
with Tim before running gate 5 (estimates have drifted ~$2.4 low before, so the true number may differ either way).
