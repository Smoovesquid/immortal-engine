...
### WIN-LOOK-1 — looking through/at a window answers from the window, not a room survey  ·  make-correct / DM_TEST pillar  ·  **QUEUED 2026-07-10 (07-10 re-gate finding; Tim: queue, do not start).** "I pocket the cord and open the shuttered window to look outside — what do I see?" → swallowed by META_LOCATION's room survey, twice running, even naming the just-opened window "shuttered" as if untouched (Rules Lawyer, verbatim gate turns). SAME shape as DM-GATE-1d's B1 (an object-look answer beaten by the room survey) but a DIFFERENT object class: B1's `carriedItemLookReferent` (gracefulAdjudication.js) is deliberately scoped to carried INVENTORY items only — a window is present furniture/an opening, never matched, never should have been by that fix. Needs: (a) recognize "look through/out/at the window" as an object-referent question about the window itself, not a survey; (b) an honest exterior answer — the engine's outdoor/settlement context for the current node is knowable (node name, neighbors, biome), so the response should ground in that rather than invent a view; (c) the "open X" state-change (windows already track open/shut per MR-2d apertures) must be reflected — an opened window should never still read "shuttered" in the same or a later turn. Likely files: engine/playloop.js (object-look routing, near the B1/carriedItemLookReferent precedent) or engine/grace/gracefulAdjudication.js; engine/structures/roomWindows.js (existing window/aperture model — MR-2d). Diagnosis-first: confirm exactly which detector currently intercepts this turn before patching.
