// U282 — Conversation honesty: the DM tells you only what you can PERCEIVE or have
...
// ── FIX 1 ────────────────────────────────────────────────────────────────────
test('U282-1a: look-around inside registers a present person — named at HOME', () => {
...
test('U282-1b: look-around inside reads an UNMET person by ROLE away from home', () => {
...
test('U282-1c: none present → no people line, but the room is still described', () => {
  const survey = buildLocationSurvey(interiorWorld({ home: 'town', npcs: [] }));
  assert.doesNotMatch(survey, /\bis here\b|\bare here\b/, survey);
  assert.match(survey, /wooden table/, survey);
});

test('U282-1d: a hostile lurker reads as a wary stranger, never a named neighbor', () => {
...
test('U282-2a: "Is anyone in the room with me?" is a meta-question (no roll path)', () => {
...
test('U282-2b: presence question answers FREE, listing present people, no [roll:]', () => {
...
test('U282-2c: presence question with no one present gives an honest "alone"', () => {
  const ans = handleMetaQuestion('is anyone here?', interiorWorld({ home: 'town', npcs: [] }));
  assert.doesNotMatch(ans, ROLL_RE, ans);
  assert.match(ans, /alone|no one/i, ans);
});

test('U282-2d: end-to-end — the presence question never rolls a WITS check', () => {
...
test('U282-3a: greeting an NPC gives approach + observable manner, NOT their inner want', () => {
