...
### MAP-WEDGE-1 — in-browser QUICKSTART world generation intermittently hard-wedges the renderer  ·  map/boot  ·  **QUEUED 2026-07-06 (observed 2/4 preview-tab boots; even `1+1` eval dead; headless same-seed generation clean ~2 s; save-continue path never wedged).** Needs a profiling packet (devtools/claude-in-chrome attach or instrumented boot timers); suspect a browser-only synchronous spin during slice generation + first map mount. Do NOT ship a blind fix.
