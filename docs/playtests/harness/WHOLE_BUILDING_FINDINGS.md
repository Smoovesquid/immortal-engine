...
### WB-Q1 — The DM invents geography the topology doesn't have (THE root coherence bug)
...
## Triage summary
- **Walking through the building now works** (WB-F1/F2/F3) — the literal ask is met.
- **The residual incoherence is one root + its symptoms:** the DM narrates a bigger,
  richer building than the topology has (**WB-Q1**), which drives the generic roll-failures
  (WB-Q2), the out-of-context free-action rolls (WB-Q3), and compounds with node-global
  furniture (WB-Q5). **WB-Q1 is the highest-leverage next packet.**
