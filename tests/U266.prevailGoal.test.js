// U266 — the combat-slice harness goal (W2·1). GOAL_PREVAIL points the Human
...
test('U266: GOAL_PREVAIL is registered in the goal table', () => {
  assert.equal(getGoal('prevail-in-fight'), GOAL_PREVAIL);
  assert.ok(GOALS['prevail-in-fight']);
});

test('U266: not satisfied while inside / outside / mid-fight', () => {
...
test('U266: satisfied only when a foe was downed, the fight is over, and the PC lives', () => {
  assert.equal(GOAL_PREVAIL.satisfied(won()), true);
  assert.equal(GOAL_PREVAIL.satisfied(died()), false, 'a downed PC has not prevailed');
});

test('U266: progress climbs inside → outside → wearing the foe down → win', () => {
